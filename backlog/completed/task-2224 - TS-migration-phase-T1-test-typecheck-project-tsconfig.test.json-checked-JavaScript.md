...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 tsconfig.test.json exists with noEmit, allowJs, checkJs and includes test/**/*.js
- [x] #2 ./scripts/verify-local.sh static-analysis runs the test typecheck as a named stage and passes
- [x] #3 npm test passes unchanged
- [x] #4 Revealed test-type defects are fixed or annotated with @ts-expect-error plus a reason
- [x] #5 Rollback: reverting the phase commit restores the previous static-analysis behavior with no runtime impact
<!-- AC:END -->
