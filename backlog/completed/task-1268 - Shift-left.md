...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The gate runs mechanically before each review round and its exit code on a pinned tree is the sole trusted signal that verification passed (agent textual claims are never trusted)
- [x] #2 On gate failure the workflow auto-bounces to the implementer with a dedicated send-back prompt without consuming a reviewer cycle
- [x] #3 Gate scope is selected from what changed (diff-scoped area) rather than always running 'all'
- [x] #4 Closes the fail-open paths identified in TASK-1306 and TASK-1335: no phase can advance on an unproven gate
- [x] #5 Regression tests cover gate-passes-advances and gate-skipped/failed-bounces-back
<!-- AC:END -->
