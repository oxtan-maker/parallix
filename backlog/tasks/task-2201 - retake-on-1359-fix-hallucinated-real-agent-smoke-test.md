...
Note: the original AC #1 called for this to be "non-blocking (nightly/on-demand, never in the merge gate)". The delivered implementation instead made it a blocking `px integrate` gate. On review, blocking is actually the right call here — a smoke test that never gates anything provides little value — so this is NOT a defect to fix. Keep it blocking in the retake.
