...
- `lib/commands` is shared with TASK-1369/1371. If `lib/commands/*.js` is not yet in `.gitignore`/`.eslintignore`, add it (with `!lib/commands/<file>.js` negations for still-hand-written commands). **Delete the negation** for each file this wave converts.
