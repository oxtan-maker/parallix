...
// Integrate reuses the single shared rebase workflow (the same one `px rebase`
// owns) so base-branch resolution (ADR 0043), conflict classification and the
// agent-assisted conflict path are implemented once. The adapter imports
// `integrate` back for its default conflict resolver; both sides only touch
// that binding at call time, so the cycle is safe at module load.
import { runRebaseWorkflow } from '../../../application/rebase-workflow.js';
import { createRebaseWorkflowPort } from '../../rebase/rebase-workflow-adapter.js';
...
async function runIntegrationRebase(
  slug: string,
  { baseWorktree, baseBranch, git, missionServicesFn }: { baseWorktree: string, baseBranch: string, git: Function, missionServicesFn: Function },
): Promise<void> {
  let rebaseExitCode: number | null = null;
  const port = createRebaseWorkflowPort({
    gitFn: git as Function,
    ...
    missionServicesFn: missionServicesFn as Function,
  ...
  port.getCurrentBranch = () => missionBranchName(slug, baseWorktree);
  port.resolveMissionBaseBranch = () => baseBranch;
  try {
    await runRebaseWorkflow([slug], port);
  } catch (error) {
    fmt.log.fail(`Integration-time rebase aborted: ${(error as any)?.message || String(error)}`);
  ...
  if (rebaseExitCode !== 0) {
    fmt.log.fail(`Integration-time rebase dead-ended with exit code ${rebaseExitCode}.`);
    throw new IntegrationAbort();
  }
  const rebaseStatus = git(['-C', baseWorktree, 'rebase', '--show-current']);
  const rebaseInProg = rebaseStatus.stdout.trim().length > 0;
  const missionSha = git(['-C', baseWorktree, 'rev-parse', missionBranchName(slug, baseWorktree)]).stdout.trim();
  ...
  if (!ancestryOk) {
    fmt.log.fail(`Integration-time rebase did not complete cleanly (inProgress=${rebaseInProg}, target=${baseSha}).`);
    throw new IntegrationAbort();
  }
  fmt.log.info(`Rebased ${fmt.branch(missionBranchName(slug, baseWorktree))} onto the primary branch. Integration gates and the probe merge now run against the rebased branch.`);
}
 ...
 */
export function predictIntegrationRebase(
  slug: string,
  { baseWorktree, baseBranch, git }: { baseWorktree: string, baseBranch: string, git: Function },
): IntegrationRebasePrediction {
...
export interface IntegrateFn extends Function {
  ...
  predictIntegrationRebase: typeof predictIntegrationRebase;
  runIntegrationRebase: typeof runIntegrationRebase;
}
...
async function integrate(args: string[], options: {
  ...
  if (!slug) {
    ...
    try {
      ...
      // see the actual merge base. Restores the step `px rebase` owns; integration
      // reuses that workflow instead of re-implementing it (task-2506). A dry run
      // only reports the need / conflict and never touches the branch.
      if (dryRun) {
...
(integrate as any).predictIntegrationRebase = predictIntegrationRebase;
(integrate as any).runIntegrationRebase = runIntegrationRebase;
