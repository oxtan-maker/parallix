...
import {
  IntegrationAbort, maybeUpdateGraphifyOnPrimary, prepareNoisePatchForSquash,
  restoreNoisePatchAfterSquash,
  resolveForgejoUserForIntegration,
  getUnresolvedIndexConflicts, areAllBacklogOnlyConflicts,
  findExistingSquashCommit, stashMainCheckoutIfNeeded,
  restoreMainCheckoutStash, rewriteWorktreePaths, cleanupMissionWorktree,
  resolveConflictsForMission, isNoMergeToAbortResult, handleHookFailureAutoBounce, classifyHookFailure,
  formatRecordedStatsRow, recordPostIntegrationStatsOrAbort,
  runPostIntegrateHookOrAbort, reportSyncMergedFailure,
  getIntegrationGatePlan, printIntegrationGatePlan, buildIntegrationGateEnv,
  captureFinalIntegrationTree, resolveIntegrationVerificationWorktree,
  buildIntegrationVerificationInvocation, executeIntegrationGates,
  buildConflictResolutionPrompt, parseStashPopCollisionFiles,
  reportStashPopFailure, SYNC_MERGED_DIAGNOSTICS, printDiagnosticTable,
  recordPostIntegrationStats, detectChangedAreas, parseFilesToAreas,
  loadIntegrationConfig, orderIntegrationGates, gateMatchesChangedAreas,
  persistLandedIntegrationOrAbort, isIntendedPayloadAtHead
} from './integrate.js';
...
function parseIntegrateArgs(args: string[]) {
  ...
  if (realAgent === 'codex' && realAgentModel !== CODEX_REAL_AGENT_MODEL) {throw new Error(`Unsupported Codex real-agent model "${realAgentModel}". Supported value: ${CODEX_REAL_AGENT_MODEL}.`);}
  if (noIntegrationGates && process.env.PARALLIX_TEST_ALLOW_INTEGRATION_GATE_BYPASS !== '1') {throw new Error('--no-integration-gates is rejected: final integration gates are mandatory.');}
...
export { integrate, parseIntegrateArgs, buildIntegrationContext, evaluateTaskStatusForIntegration, printMergedPrRecoveryGuidance, promoteTaskForIntegrationIfNeeded, printIntegrationPreflight, VARIANT_B_AUTOMATION_SUMMARY, REAL_AGENT_OPTION, REAL_AGENT_MODEL_OPTION, INTEGRATE_VALUE_OPTIONS, CODEX_REAL_AGENT_MODEL };
