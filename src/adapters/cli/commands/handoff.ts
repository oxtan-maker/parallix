...
/**
  ...
  */
  ...
  if (checkpoints.length === 0) {
    // Auto-remediation (architecture migration): rather than hard-failing when no checkpoint
...
/**
 * Build the content for an auto-generated CP-1.md checkpoint (architecture migration).
 *
...
async function captureNelAtHandoff(slug, options) {
  ...
  let nelRecord;
  try {
    nelRecord = computeNELRecord(`${primaryBranch}..HEAD`, { cwd: rootDir });
  } catch (_) {
  ...
  if (typeof options.missionServicesFn !== 'function') { return { ok: false, error: 'mission services are not configured' }; }
  const missionServices = await options.missionServicesFn(rootDir, {
    missionDir,
...
/** @param {string[]} args */
async function handoffCommand(args, options = {}) {
  ...
  const result = await _exports.performHandoff(slug, { ...options, skipGate, force });
  if (!result.ok) {
    process.exit(1);
  }
}
...
const _namedExports = {
  ...
  // Test seams (architecture migration): expose the auto-checkpoint template and the
