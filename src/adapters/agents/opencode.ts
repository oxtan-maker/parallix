...
import { detectLimitHit } from '../../application/services/agent-limit.js';
...
//      zeros (architecture migration). We request `--format json` in buildOpencodeInvocation
...
function buildOpencodeInvocation({ prompt, worktree, env, resume = false, sessionId = null, model = null, preferJson = true }: BuildOpencodeInvocationOptions) {
  ...
  // telemetry capture (architecture migration).
  //
