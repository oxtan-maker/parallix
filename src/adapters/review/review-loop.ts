...
async function markStageLaunchRecorded(
  ...
  await persistReviewStateOrThrow(writeReviewStateFn, slug, state as ReviewState, worktree || process.cwd());
  return true;
}
...
// ============================================================================
// Pre-review Gate Enforcement (ADR 0048 Control C1 / architecture migration)
// ============================================================================
...
export async function handleGateFailureAutoBounce(
  ...
  if (persisted) {
    ...
    await persistReviewStateOrThrow(writeReviewStateFn, slug, updatedState as any, worktree);
  } else {
    await persistReviewStateOrThrow(writeReviewStateFn, slug, { metadata } as any, worktree);
  }
...
export async function startReviewLoop(slug: string, opts: {
  ...
  // `--mission <path>` override (architecture migration architecture invariant): when the caller points the
  ...
  if (!dryRun && forgejoEnabled) {
    ...
    if (!pr.exists || pr.state !== 'open') {
      ...
      // create PR, reliable since architecture migration), re-check, and continue the loop once a
  ...
  for (let attempt = initialRound; attempt <= maxAttempts; attempt++) {
    ...
    // baseline exists to suppress (architecture migration).
    ...
    if (state.phase === 'reviewing') {
      ...
      if (dryRun) {
        ...
        if (!reviewState) {
          ...
          // with the SHA that HEAD was actually rebased onto (architecture migration).
    ...
    // Fixing phase
    // architecture migration: snapshot the mission branch HEAD at this round/commit boundary
    ...
    // architecture migration: track whether the implementer was skipped on resume so the
    ...
    if (!disposition) {
      ...
      // architecture migration: implementer was skipped on resume; the HEAD snapshot was
    ...
    state.disposition = disposition as string;
    try { state.transitionTo('reviewing'); } catch (_) { /* ignore */ }
    // Push mission branch to review remote so the PR diff is current (architecture migration)
    // Skip if no new committed source change (architecture invariant: activated-Forgejo + CHANGES_MADE
    ...
    const hasCommittedChange = hasNewCommittedChangeFn
      ...
      : (() => {
          // architecture migration (F1 round 4): when the implementer was skipped on resume,
