...
const SHIPPED_STATE_MAP_PATH = 'config/state-map.json';
...
export function loadStateMap(options: StateMapOptions = {}): Record<string, unknown> {
  ...
  try {
    const content = stateMapPath === SHIPPED_STATE_MAP_PATH
      ? runtimeAssetStore.readText(SHIPPED_STATE_MAP_PATH)
      : fs.readFileSync(stateMapPath, 'utf8');
