...
function evidenceCellHasVerifiableReference(cell: string, rootDir: string, knownTestNames: Set<string>): boolean {
  ...
  while ((fileLineMatch = fileLinePattern.exec(normalized)) !== null) {
    ...
    // Mission checkpoints predate the runtime-source migration and may cite
    // `lib/...` paths. Keep those historical references verifiable against
    // their canonical TypeScript locations in a source checkout.
