...
export async function run(argv = process.argv.slice(2), options: RunOptions = {}): Promise<number> {
  ...
  try {
    const [missionStartModule, reviewEventsModule, workflow] = await Promise.all([
      import('./lib/commands/mission-start.js'),
      import('./lib/review/review-events.js'),
      import('./index.js'),
    ]);
    const missionStart = missionStartModule.default;
    const { createEvent } = reviewEventsModule;
