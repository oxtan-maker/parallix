        ...
        if (!reviewState) {
          ...
          if (reviewer === 'autonomous' && !forgejoEnabled) {
            ...
            recordStageStatsSafe('review', {
              ...
              sinceMs: (reviewerLaunchResult && reviewerLaunchResult.result && reviewerLaunchResult.result.startedAt)
                ? Date.parse(reviewerLaunchResult.result.startedAt)
                : 0, log, error
            });
          }
    ...
    if (!disposition) {
      ...
      if (implementer === 'autonomous' && !forgejoEnabled) {
        ...
        recordStageStatsSafe('active', {
          ...
          sinceMs: (implementerLaunchResult && implementerLaunchResult.result && implementerLaunchResult.result.startedAt)
            ? Date.parse(implementerLaunchResult.result.startedAt)
            : 0, log, error
        });
      }
