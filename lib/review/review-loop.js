...
// one launch at a time, de-duped via review-state metadata, so Claude/Qwen rows
...
function recordStageStatsSafe(kind, { stage, slug, rootDir, worktree, implementer, reviewer, result, sinceMs, log, error, state, writeReviewStateFn = writeReviewState }) {
  ...
  try {
    ...
    stats.accumulateStageStats({ stage, slug, rootDir, implementer, reviewer, telemetry, durationMinutes });
  } catch (err) {
