 ...
 */
function eventFilePath(slug, eventType, round, actor, /** @type {string|null} */ timestamp = null, rootDir = process.cwd()) {
 ...
 */
function isValidEventType(/** @type {string} */ eventType) {
  return ALL_EVENT_TYPES.includes(/** @type {typeof ALL_EVENT_TYPES[number]} */ (eventType));
}
 ...
 */
function shouldMirrorToProvider(/** @type {string} */ eventType) {
  return MIRRORED_EVENT_TYPES.has(/** @type {typeof MIRRORED_EVENT_TYPES extends Set<infer T> ? T : string} */ (eventType));
}
 ...
 * @returns {{eventType: string, timestamp: string, content: string}}
 */
function normalizeEventContent(content, eventType, metadata = {}) {
  /** @type {{eventType: string, timestamp: string, content: string, [key: string]: any}} */
  const payload = { eventType, timestamp: '', content, ...metadata };
 ...
 */
/**
 * @param {{eventType: string, timestamp: string, round?: number, phase?: string, actor?: string, slug?: string, disposition?: string, verdict?: string, fixedItems?: string[], pushedBackItems?: string[], parkedItems?: string[], blockedReason?: string, followUpReference?: string}} event
 * @returns {string}
 */
 ...
 */
function classifyComment(/** @type {{[key: string]: any}} */ comment) {
 ...
 * @param {string} slug
 * @param {string} actor
 * @param {{getCommentsFn?: Function, createEventFn?: Function, readTokenFn?: Function, reviewIdentity?: string|null, forgejoUser?: string|null, worktree?: string, log?: Function, error?: Function}} [options]
 * @returns {Promise<{ok: boolean, created: Array<any>, skipped: Array<any>, error?: string}>}
 */
 ...
 */
/**
 * @param {{content?: string, round?: number, phase?: string, slug?: string, eventType?: string, timestamp?: string}} event
 * @returns {string}
 */
function renderEventFile(event) {
  const frontmatter = buildEventFrontmatter(/** @type {{eventType: string, timestamp: string, content?: string, round?: number, phase?: string, slug?: string}} */ (event));
  const content = /** @type {string} */ (event.content || '');
  ...
  const footer = event.round !== undefined && event.phase !== undefined
    ? buildEventFooter(/** @type {string} */ (event.slug), event.round, event.phase)
    : '';
 ...
 * @param {Array<any>} [params.fixedItems] - For implementer round summary
 * @param {Array<any>} [params.pushedBackItems] - For implementer round summary
 * @param {Array<any>} [params.parkedItems] - For implementer round summary
 ...
 * @param {string} slug
 * @param {string} eventType
 * @param {{content: string, round?: number, phase?: string, actor?: string, disposition?: string, verdict?: string, fixedItems?: Array<any>, pushedBackItems?: Array<any>, parkedItems?: Array<any>, blockedReason?: string, followUpReference?: string, timestamp?: string}} params
 * @param {{skipGit?: boolean, gitFn?: Function, log?: Function, error?: Function, worktree?: string, allowMissingRequiredFields?: boolean}} [options]
 * @returns {{ok: boolean, path: string|null, error?: string|null, event?: Object}}
 */
 ...
 * @param {string} eventType
 * @param {{round?: number, phase?: string, actor?: string}} [params]
 * @param {{tmpDir?: string, log?: Function, error?: Function, worktree?: string}} [options]
 * @returns {{ok: boolean, path: string|null, importedFrom: string|null, event?: Object, skipped?: boolean, legacyPath?: string, error?: string}}
 */
function importLegacyArtifact(slug, artifactName, eventType, params = {}, options = {}) {
  ...
  try {
    ...
    error(fmt.status('FAIL', `Failed to read legacy artifact: ${/** @type {Error} */(err).message}`));
    return { ok: false, path: null, importedFrom: legacyPath, error: /** @type {Error} */ (err).message };
  }
  ...
  const result = createEvent(slug, eventType, { 
    ...
    actor: params.actor || (/** @type {Function} */ (inferActorFromArtifact))()
  }, { ...options, worktree: worktree || undefined, allowMissingRequiredFields: true });
  
  if (!result.ok) {
    return { ok: false, path: null, importedFrom: legacyPath, error: /** @type {string | undefined} */ (result.error) };
  }
  
  log(fmt.status('PASS', `Imported legacy artifact ${artifactName} to ${path.basename(/** @type {string} */ (result.path))}`));
...
/**
 * @param {string} slug
 * @param {{tmpDir?: string, error?: Function, worktree?: string, log?: Function}} [options]
  * @returns {{ok: boolean, imported: Array<any>, errors: Array<any>}}
 */
function importAllLegacyArtifacts(slug, options = {}) {
  ...
  if (outcomePath && fs.existsSync(outcomePath)) {
    ...
    if (verdictPath) {
      try {
        ...
        error(fmt.status('WARN', `Failed to read verdict file: ${/** @type {Error} */(err).message}`));
      }
    }
    
    // Import review-outcome.md with verdict as metadata (only include verdict if valid)
    /** @type {{round: number, phase: string, actor: string, verdict?: string}} */
    const outcomeParams = { round, phase, actor: 'legacy' };
    if (verdictContent && isValidVerdict(/** @type {string} */ (verdictContent))) {
  ...
  for (const artifactName of Object.keys(LEGACY_ARTIFACT_TO_EVENT_TYPE)) {
    ...
    const result = importLegacyArtifact(slug, artifactName, /** @type {string} */ (LEGACY_ARTIFACT_TO_EVENT_TYPE[/** @type {keyof typeof LEGACY_ARTIFACT_TO_EVENT_TYPE} */ (artifactName)]), 
      { round, phase, actor: 'legacy' }, 
      { ...options, worktree: rootDir });
    
    if (result.ok) {
      if (result.path) {
        imported.push({ artifactName, eventType: LEGACY_ARTIFACT_TO_EVENT_TYPE[/** @type {keyof typeof LEGACY_ARTIFACT_TO_EVENT_TYPE} */ (artifactName)], path: result.path, legacyPath: result.legacyPath });
      }
    } else {
      errors.push({ artifactName, eventType: LEGACY_ARTIFACT_TO_EVENT_TYPE[/** @type {keyof typeof LEGACY_ARTIFACT_TO_EVENT_TYPE} */ (artifactName)], error: result.error });
    }
  }
 ...
 * @returns {Array<any>} - Array of parsed event objects
 */
/**
 * @param {string} slug
 * @param {{rootDir?: string, readdirSync?: Function, readFileSync?: Function, error?: Function}} [options]
 * @returns {Array<any>}
 */
function readAllEvents(slug, options = {}) {
  ...
  try {
    ...
    error(fmt.status('WARN', `Failed to read review-events directory: ${/** @type {Error} */(err).message}`));
    return [];
  }
  ...
  for (const filename of files) {
    ...
    try {
      ...
      error(fmt.status('WARN', `Failed to read event file ${filename}: ${/** @type {Error} */(err).message}`));
      continue;
    }
  ...
  if (frontmatterMatch) {
    ...
    for (const line of lines) {
      ...
      if ((value.startsWith('"') && value.endsWith('"')) || 
        ...
        /** @type {any} */ (metadata)[key] = value.slice(1, -1);
      }
      // Handle JSON arrays
      else if (value.startsWith('[') && value.endsWith(']')) {
        try {
          /** @type {any} */ (metadata)[key] = JSON.parse(value);
        } catch (_) {
          /** @type {any} */ (metadata)[key] = value;
        }
      }
      // Handle booleans and numbers
      else if (value === 'true') {/** @type {any} */ (metadata)[key] = true;}
      else if (value === 'false') {/** @type {any} */ (metadata)[key] = false;}
      else if (value !== '' && !isNaN(Number(value))) {/** @type {any} */ (metadata)[key] = Number(value);}
      else {/** @type {any} */ (metadata)[key] = value;}
    }
  }
