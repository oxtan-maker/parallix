...
exports.readConfiguredReviewRemote = readConfiguredReviewRemote;
exports.setupReview = setupReview;
exports.setupWizard = setupWizard;
exports.apiRequest = apiRequest;
exports.buildWorkflowConfig = buildWorkflowConfig;
exports.collectWizardAnswers = collectWizardAnswers;
exports.defaultProductName = defaultProductName;
exports.defaultRepoSlug = defaultRepoSlug;
exports.parseYesNo = parseYesNo;
exports.runVerifyEnv = runVerifyEnv;
exports.bootstrapReviewSurface = bootstrapReviewSurface;
exports.collectSetupAnswers = collectSetupAnswers;
exports.createToken = createToken;
exports.ensureForgejoUser = ensureForgejoUser;
exports.ensureRepo = ensureRepo;
exports.ensureReviewRemote = ensureReviewRemote;
exports.evaluateReviewSetup = evaluateReviewSetup;
...
exports.writeWorkflowConfig = writeWorkflowConfig;
exports.resolveBootstrapForgejoHome = resolveBootstrapForgejoHome;
exports.buildNonInteractiveAnswers = buildNonInteractiveAnswers;
 ...
 * @returns {string}
 */
function detectPrimaryBranchDefault(rootDir, options = {}) {
    const { spawnSyncFn = spawnSync, configuredPrimaryBranch = null } = options;
    const listResult = spawnSyncFn('git', ['-C', rootDir, 'branch', '--list', '--format=%(refname:short)', 'main', 'master'], { encoding: 'utf8' });
...
function buildWorkflowConfig(answers) {
    ...
    if (typeof answers.primaryBranch === 'string' &&
...
function suggestedForgejoUsers() {
    return unique([
        ...(0, agents_1.eligibleAgentsForStep)('active'),
        ...(0, agents_1.eligibleAgentsForStep)('review'),
    ]);
}
/** @param {string} user @param {string} [forgejoHome] @returns {string} */
function tokenFilePath(user, forgejoHome) {
    return path.join(forgejoHome || (0, forgejo_1.resolveForgejoHome)(), 'tokens', user);
}
/** @param {string} [forgejoHome] @returns {string} */
function ensureTokenDir(forgejoHome) {
    const dir = path.join(forgejoHome || (0, forgejo_1.resolveForgejoHome)(), 'tokens');
    fs.mkdirSync(dir, { recursive: true });
    return dir;
}
/** @param {string} [rootDir] @param {string} [explicitForgejoHome] @returns {string} */
function resolveBootstrapForgejoHome(rootDir, explicitForgejoHome) {
    if (explicitForgejoHome) {
        return explicitForgejoHome;
    }
    return path.join(rootDir || process.cwd(), '.forgejo-local');
}
function readConfiguredReviewRemote(rootDir, remoteName) {
 ...
 */
function evaluateReviewSetup(rootDir, options = {}) {
    const { users = suggestedForgejoUsers(), remoteName, remoteUrlFn = forgejo_1.reviewRemoteUrl, getRemoteUrlFn = readConfiguredReviewRemote, tokenPathFn = tokenFilePath, requestFn = apiRequest, } = options;
    const reviewAdapter = (0, product_config_1.resolveReviewAdapter)(rootDir);
    ...
    const review = (0, forgejo_1.resolveForgejoSettings)(rootDir);
 ...
 */
function ensureForgejoUser(baseUrl, user, ownerLogin, ownerPassword, password, requestFn = apiRequest, options = {}) {
    const normalizedBaseUrl = normalizeBaseUrl(baseUrl);
 ...
 */
async function collectSetupAnswers(rootDir, options = {}) {
    const { promptFn = promptLine, log = fmt.log.info, users = suggestedForgejoUsers(), resolveForgejoSettingsFn = forgejo_1.resolveForgejoSettings, } = options;
 ...
 */
async function collectWizardAnswers(rootDir, options = {}) {
    const { promptFn = promptLine, log = fmt.log.info, users = suggestedForgejoUsers(), loadWorkflowConfigFn = product_config_2.loadWorkflowConfig, detectPrimaryBranchDefaultFn = detectPrimaryBranchDefault, } = options;
    ...
    const useDefaults = parseYesNo(await promptFn('Use the standard markdown task workflow layout and defaults? [Y/n]: '), true);
    ...
    if (!useDefaults) {
        log(fmt.status('INFO', 'Verification command runs the primary gate. Put the {{area}} token where the detected ' +
    ...
    const bootstrapReview = parseYesNo(await promptFn('Create/update the Forgejo repo, token files, and review remote now? This will ask for the repo-owner password and optional agent passwords. [Y/n]: '), true);
...
function writeToken(user, token, forgejoHome) {
...
function tokenCreateViaOwnerToken(baseUrl, repoSlug, ownerToken, agentPasswords, repoInfo, options = {}) {
    ...
    const normalizedBaseUrl = normalizeBaseUrl(baseUrl);
    const forgejoHome = typeof injectedForgejoHome === 'string' ? injectedForgejoHome : (0, forgejo_1.resolveForgejoHome)();
    const createdTokens = [];
    const warnings = [];
 ...
 */
async function bootstrapReviewSurface(rootDir, setup, options = {}) {
    const { log = fmt.log.info, promptFn, requestFn = apiRequest, maxPasswordAttempts = 3, interactive = true, reviewRemoteUrlFn = forgejo_1.reviewRemoteUrl, } = options;
    ...
    if (interactive) {
        ...
        if (!ownerTokenResult.ok) {
            return ownerTokenResult;
        }
        ...
        for (const agent of setup.agentPasswords) {
            if (agent.password) {
                const userResult = ensureForgejoUser(baseUrl, agent.user, setup.ownerLogin, setup.ownerPassword, agent.password, requestFn);
                if (!userResult.ok) {
        ...
        const remoteName = (0, product_config_1.resolveReviewAdapter)(rootDir).remote || 'review';
    ...
    const remoteUrl = (0, forgejo_1.reviewRemoteUrl)(rootDir);
    const remoteName = (0, product_config_1.resolveReviewAdapter)(rootDir).remote || 'review';
    const remoteResult = remoteUrl ? ensureReviewRemote(rootDir, remoteName, remoteUrl) : { ok: true, created: false, updated: false };
    if (!remoteResult.ok) {
        return remoteResult;
