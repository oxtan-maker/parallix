...
function syncPrimaryBaseline(user, token, rootDir = process.cwd()) {
  ...
  const primaryExists = git.git(['-C', rootDir, 'show-ref', '--verify', '--quiet', `refs/heads/${primaryBranch}`]);
  ...
  const result = git.git(['-C', rootDir, 'push', '--force', remoteUrl, `${primaryBranch}:${primaryBranch}`], {
    stdio: 'pipe'
  });
