...
exports.resolveForgejoHome = resolveForgejoHome;
exports.isForgejoPath = isForgejoPath;
exports.resolveTokenFile = resolveTokenFile;
exports.readToken = readToken;
exports.forgejoApi = forgejoApi;
exports.forgejoApiAsync = forgejoApiAsync;
exports.getPrNumber = getPrNumber;
exports.getPrAuthor = getPrAuthor;
exports.listOpenPrsForSlug = listOpenPrsForSlug;
exports.isApiErrorResult = isApiErrorResult;
exports.getLatestReview = getLatestReview;
exports.getLatestReviewForPr = getLatestReviewForPr;
exports.getLatestReviewDecision = getLatestReviewDecision;
exports.getLatestDisposition = getLatestDisposition;
exports.getLatestDispositionForPr = getLatestDispositionForPr;
exports.postComment = postComment;
exports.forgejoAvailable = forgejoAvailable;
exports.postReview = postReview;
exports.syncMerged = syncMerged;
exports.pushReviewRef = pushReviewRef;
exports.isStaleInfoPushRejection = isStaleInfoPushRejection;
exports.fetchReviewBranch = fetchReviewBranch;
exports.deleteReviewRef = deleteReviewRef;
exports.verifyCommitExists = verifyCommitExists;
exports.remoteRefContainsCommit = remoteRefContainsCommit;
exports.resolveTrackingBranchSha = resolveTrackingBranchSha;
exports.deriveRepoFromGitRemote = deriveRepoFromGitRemote;
exports.resolveForgejoSettings = resolveForgejoSettings;
exports.reviewRemoteUrl = reviewRemoteUrl;
exports.authenticatedReviewUrl = authenticatedReviewUrl;
exports.syncPrimaryBaseline = syncPrimaryBaseline;
exports.ensureRemoteBaseBranch = ensureRemoteBaseBranch;
exports.createPr = createPr;
exports.getCommentsSync = getCommentsSync;
...
const git_js_1 = require("../core/git.js"); // NOTE: Original JS used `const git = require('../core/git')` and called `git.git(...)`. TS migration extracts the callable `git` function directly; `git(args, opts)` is functionally equivalent to the old `git.git(args, opts)`.
...
function deriveRepoFromGitRemote(rootDir, remoteName) {
    ...
    try {
        const result = (0, child_process_1.spawnSync)('git', ['-C', rootDir, 'remote', 'get-url', remote], {
            encoding: 'utf8',
            timeout: 2000,
        });
        if (result.status !== 0) {
...
function listGitWorktrees(rootDir = process.cwd()) {
    try {
        const result = (0, child_process_1.spawnSync)('git', ['-C', rootDir, 'worktree', 'list', '--porcelain'], {
            encoding: 'utf8',
            timeout: 2000,
        });
        if (result.status !== 0) {
...
function resolveForgejoHome(rootDir = process.cwd()) {
    if (process.env.FORGEJO_HOME) {
        return process.env.FORGEJO_HOME;
    }
    ...
    const candidates = [directLocal];
    const seen = new Set([directLocal]);
    ...
    try {
        const main = (0, mission_utils_js_1.getPrimaryWorktree)();
        ...
        try {
            ...
            for (const entry of fs.readdirSync(parentDir)) {
                if (entry !== repoBase && !entry.startsWith(`${repoBase}-`)) {
                    continue;
                }
...
function isForgejoPath(targetPath, options = {}) {
    ...
    if (!normalizedTarget || !normalizedForgejoHome) {
        return false;
    }
...
function resolveForgejoSettings(rootDir = process.cwd()) {
    const review = (0, product_config_js_1.resolveReviewAdapter)(rootDir);
 ...
 */
function getPrStatus(branch, rootDir, options = {}) {
    /** @type {{forgejoUser?: string, token?: string, apiCall?: Function}} */
    const { forgejoUser, token: providedToken, apiCall = forgejoApi } = options;
 ...
 * @returns {string|null}
 */
function resolveTokenFile(user, rootDir = process.cwd()) {
 ...
 * @returns {string|null}
 */
function readToken(user, rootDir = process.cwd()) {
    const resolvedUser = resolveForgejoUser(user);
 ...
 */
async function forgejoApiAsync(method, apiPath, token, body, options = {}) {
    /** @type {{rootDir?: string, timeout?: number}} */
    const { rootDir = process.cwd(), timeout = HTTP_REQUEST_TIMEOUT } = options;
    const { url: forgejoUrl, repo: forgejoRepo } = resolveForgejoSettings(rootDir);
...
function createPr(branch, user, token, options = {}) {
    const { rootDir = process.cwd(), apiCall = forgejoApi, log = fmt.log.info, force = false, forceWithLease = false, verificationArea = null, captureVerifiedTreeProofFn = verification.captureVerifiedTreeProof, assertVerifiedTreeProofFn = verification.assertVerifiedTreeProof } = options;
    let primaryBranch = 'main';
    try {
        primaryBranch = (0, mission_utils_js_1.getPrimaryBranch)(rootDir);
    }
    catch (_) {
        primaryBranch = 'main';
    }
    if (branch === primaryBranch) {
        return { ok: false, error: `cannot create a PR from ${primaryBranch}` };
    }
    ...
    if (slug) {
        try {
            const resolvedBase = (0, mission_utils_js_1.resolveMissionBaseBranch)(slug, rootDir);
    ...
    let pushResult = (0, git_js_1.git)(pushArgs, { stdio: ['ignore', 'pipe', 'pipe'], env: cLocaleEnv() });
    ...
    if (pushResult.status !== 0) {
        if (forceWithLease && isStaleInfoPushRejection(pushResult)) {
            ...
            pushResult = (0, git_js_1.git)(pushArgs, { stdio: ['ignore', 'pipe', 'pipe'], env: cLocaleEnv() });
            if (pushResult.stdout) {
                process.stdout.write(pushResult.stdout);
            }
            if (pushResult.stderr) {
...
function getPrAuthor(branch, token, options = {}) {
    ...
    const { apiCall = forgejoApi, resolvePrNumber = getPrNumber, forgejoUser, rootDir = process.cwd() } = options;
    const slugMatch = branch.match(/^mission\/(task-\d+)/);
    const slug = slugMatch ? slugMatch[1] : null;
    const prNumber = resolvePrNumber(branch, token, { apiCall, slug, forgejoUser, rootDir });
    if (isApiErrorResult(prNumber) || !prNumber) {
        return null;
    }
    const prRes = apiCall('GET', `/pulls/${prNumber}`, token, undefined, { rootDir });
    if (!prRes.ok || !prRes.data || !prRes.data.user) {
        return null;
    }
...
function resolvePrAccess(branch, token, options = {}) {
    const { apiCall = forgejoApi, pageSize = 50, maxPages = 50, slug = null, onlyOpen = false, forgejoUser, rootDir = process.cwd() } = options;
    ...
    if (prNumber) {
        return { prNumber, token: token || undefined };
    }
    const currentUser = resolveForgejoUser(forgejoUser);
    const triedUsers = [currentUser];
    // 2. Fallback for slugs
    if (slug) {
        const taskFile = (0, backlog_js_1.findTaskFile)(slug, rootDir);
        const implementer = taskFile ? (0, backlog_js_1.getTaskImplementer)(taskFile) : null;
        const repoOwner = resolveForgejoSettings(rootDir).repo.split('/')[0] || null;
        const candidates = [implementer, repoOwner, DEFAULT_FORGEJO_USER].filter((u) => Boolean(u) && u !== currentUser);
        for (const user of candidates) {
            ...
            if (fallbackToken) {
                ...
                if (prNumber) {
                    return { prNumber, token: fallbackToken };
                }
            }
        }
    }
    if (options.reportNotFound) {
        const curlCheck = (0, child_process_1.spawnSync)('curl', ['--version'], { encoding: 'utf8' });
...
function listOpenPrsForSlug(baseSlug, token, options = {}) {
    ...
    const { apiCall = forgejoApi, pageSize = 50, maxPages = 2 } = options;
    const prs = [];
    let consecutiveErrors = 0;
    for (let page = 1; page <= maxPages; page += 1) {
 ...
 * @returns {string}
 */
function authenticatedReviewUrl(user, token, rootDir) {
    const { url: forgejoUrl, repo: forgejoRepo } = resolveForgejoSettings(rootDir);
...
function reviewRemoteUrl(rootDir) {
    const { url: forgejoUrl, repo: forgejoRepo } = resolveForgejoSettings(rootDir);
    if (!forgejoUrl || !forgejoRepo) {
        return null;
    }
    const url = new URL(forgejoUrl);
 ...
 */
function syncPrimaryBaseline(user, token, rootDir = process.cwd(), { verificationProof = null, assertVerifiedTreeProofFn = verification.assertVerifiedTreeProof, gitRunner = git_js_1.git } = {}) {
    let primaryBranchName = 'main';
    try {
        primaryBranchName = (0, mission_utils_js_1.getPrimaryBranch)(rootDir);
    }
    catch (_) {
 ...
 */
function ensureRemoteBaseBranch(baseBranch, user, token, rootDir = process.cwd(), { gitRunner = git_js_1.git } = {}) {
 ...
 * @returns {*}
 */
function pushReviewRef(sourceRef, destinationRef, rootDir = process.cwd(), options) {
    const { force = false, forceWithLease = false, user, token } = options || {};
    const remote = token
    ...
    const result = (0, git_js_1.git)(pushArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
    if (result.stdout) {
...
function fetchReviewBranch(branch, rootDir = process.cwd(), options = {}) {
    ...
    const source = token
        ? authenticatedReviewUrl(user || resolveForgejoUser(), token, rootDir)
        : 'review';
    return (0, git_js_1.git)(['-C', rootDir, 'fetch', source, `+refs/heads/${branch}:refs/remotes/review/${branch}`], {
...
function resolveTrackingBranchSha(branch, rootDir = process.cwd()) {
    ...
    for (const ref of candidateRefs) {
        const result = (0, git_js_1.git)(['-C', rootDir, 'rev-parse', '--verify', `${ref}^{commit}`], {
            stdio: ['ignore', 'pipe', 'pipe']
        });
...
function buildCreatePrPushArgs(branch, remoteUrl, rootDir = process.cwd(), options = {}) {
    ...
    const { force = false, forceWithLease = false, gitFetch = fetchReviewBranch, refreshTrackingRef = false } = options;
 ...
 * @returns {*}
 */
function deleteReviewRef(branch, rootDir = process.cwd(), options) {
    const { user, token } = options || {};
    const remote = token
        ? authenticatedReviewUrl(user || resolveForgejoUser(), token, rootDir)
        : 'review';
    const result = (0, git_js_1.git)(['-C', rootDir, 'push', remote, '--delete', branch], {
        stdio: ['ignore', 'pipe', 'pipe']
    });
    if (result.stdout) {
        process.stdout.write(result.stdout);
...
function verifyCommitExists(commit, rootDir = process.cwd()) {
    return (0, git_js_1.git)(['-C', rootDir, 'rev-parse', '--verify', `${commit}^{commit}`], {
        stdio: ['ignore', 'pipe', 'pipe']
    });
}
/** @param {string} commit @param {string} [remoteRef] @param {string} rootDir @returns {*} */
function remoteRefContainsCommit(commit, remoteRef = `refs/remotes/review/${(0, mission_utils_js_1.getPrimaryBranch)(process.cwd())}`, rootDir = process.cwd()) {
    return (0, git_js_1.git)(['-C', rootDir, 'merge-base', '--is-ancestor', commit, remoteRef], {
        stdio: ['ignore', 'pipe', 'pipe']
    });
}
...
function getLatestReviewDecision(branch, options = {}) {
    ...
    const { forgejoUser, token: providedToken, apiCall = forgejoApi, rootDir = process.cwd() } = options;
    const slugMatch = branch.match(/^mission\/(task-\d+)/);
    const slug = slugMatch ? slugMatch[1] : null;
    const { token } = resolveForgejoAuth({ forgejoUser, token: providedToken, rootDir });
    if (!token) {
...
async function getLatestReviewForPr(prNumber, reviewerUser, sinceIso, token, options = {}) {
    ...
    const { apiCall = forgejoApiAsync } = options;
...
function postComment(branch, token, body, options = {}) {
    ...
    const { apiCall = forgejoApi, resolvePrNumber = getPrNumber, forgejoUser } = options;
    const slugMatch = branch.match(/^mission\/(task-\d+)/);
    const slug = slugMatch ? slugMatch[1] : null;
    ...
    if (!prNumber) {
        return { ok: false, data: null, status: null, error: 'pr-not-found' };
    }
...
function postReview(branch, token, outcome, summary, options = {}) {
    ...
    const event = /** @type {'APPROVED'|'REQUEST_CHANGES'|'COMMENT'|undefined} */ (REVIEW_OUTCOME_MAP[outcome]);
    if (!event) {
        return { ok: false, data: null, status: null, error: `unsupported-outcome: ${outcome}` };
    }
...
function syncMerged(branch, mergedCommit, options = {}) {
    ...
    const { forgejoUser, rootDir = process.cwd(), token: providedToken, baseBranch = null, apiCall = forgejoApi, resolvePrNumber = getPrNumber, gitPush = pushReviewRef, gitFetch = fetchReviewBranch, gitContainsCommit = remoteRefContainsCommit, gitDelete = deleteReviewRef, verifyCommit = verifyCommitExists, log = fmt.log.info } = options;
    if (!branch) {
    ...
    const verifyMergeState = (/** @type {number} */ statusCode) => {
        ...
        const shaMatch = (/** @type {string|null} */ a, /** @type {string|null} */ b) => a && b && (a === b || b.startsWith(a));
...
function getCommentsSync(branch, token, options = {}) {
    ...
    const { apiCall = forgejoApi, forgejoUser, rootDir = process.cwd(), log: logger = noopLog } = options;
    const slugMatch = branch.match(/^mission\/(task-\d+)/);
    const slug = slugMatch ? slugMatch[1] : null;
    const prAccess = resolvePrAccess(branch, token, { apiCall, slug, forgejoUser, rootDir });
    if (prAccess && isApiErrorResult(prAccess)) {
        logger(`getComments API error resolving PR for ${branch}: status=${(prAccess._apiError || {}).status || 0}`);
        return null;
    }
    if (!prAccess) {
        return null;
 ...
 */
async function closePr(branch, token) {
    const slugMatch = branch.match(/^mission\/(task-\d+)/);
    const slug = slugMatch ? slugMatch[1] : null;
