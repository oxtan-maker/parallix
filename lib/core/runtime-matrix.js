...
/**
 * @param {string} agent
 * @param {{workflowLauncherStatusFn?: typeof workflowLauncherStatus}} [options]
 */
...
/**
 * @param {{step?: string, eligibleAgentsForStepFn?: typeof eligibleAgentsForStep, workflowLauncherStatusFn?: typeof workflowLauncherStatus, configPath?: string, existsSyncFn?: typeof fs.existsSync}} [options]
 */
function buildAutonomousReviewMatrix(options = {}) {
  ...
  const launchers = Object.fromEntries(
    agents.map((/** @type {string} */ agent) => [agent, workflowLauncherStatusFn(agent)])
  );
...
/** @param {{step: string, agents: string[], configPath: string, configPresent: boolean, launchers: Record<string, any>}} matrix */
 ...
 * @param {{step?: string, eligibleAgentsForStepFn?: typeof eligibleAgentsForStep, workflowLauncherStatusFn?: typeof workflowLauncherStatus}} [options]
 */
function runnableDifferentFamilyExists(implementer, options = {}) {
  ...
  const agents = eligibleAgentsForStepFn(step);
  return agents.some((/** @type {string} */ a) => a !== implementer && workflowLauncherStatusFn(a).supported);
}
