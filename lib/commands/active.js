...
const EXECUTE_PROMPT_PATH = path.join((0, package_root_js_1.packageRoot)(__dirname), 'prompts', 'execute.md');
/**
 * @param {string[]} args
...
async function attemptAgentRelaunch(slug, worktree, errorMsg, agent, options = {}) {
    const { isRelaunchableErrorFn = repairHandoff.isRelaunchableError, buildRelaunchPromptFn = repairHandoff.buildRelaunchPrompt, workflowLauncherStatusFn = agents.workflowLauncherStatus, startAgentFn = agents.startAgent, log = fmt.log.plain, error = fmt.log.plainError, gateOutput, promptOverride } = options;
