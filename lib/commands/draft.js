...
const DRAFT_PROMPT_PATH = path.join((0, package_root_js_1.packageRoot)(__dirname), 'prompts', 'draft.md');
const MISSION_SCAFFOLD_PATH = path.join((0, package_root_js_1.packageRoot)(__dirname), 'templates', 'mission-scaffold.md');
...
actual, 
// @ts-expect-error implicit any binding elements
taskResolution, log = fmt.log.plain, transitionTaskFn = backlog_js_1.transitionTask, getTaskStatusFn = backlog_js_1.getTaskStatus, 
// @ts-expect-error implicit any binding elements
slug, 
// @ts-expect-error implicit any binding elements
worktree }) {
    ...
    if (!currentStatus || !transitionTaskFn(slug, currentStatus, { implementer: actual, rootDir: worktree || (0, mission_utils_js_1.resolveWorktree)(slug) || process.cwd(), log })) {
