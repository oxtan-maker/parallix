...
async function rebase(args, { inferSlugFn = mission_utils_js_1.inferSlug, findMissionDirFn = mission_utils_js_1.findMissionDir, findMissionAreaFn = mission_utils_js_1.findMissionArea, getCurrentBranchFn = git_js_1.getCurrentBranch, resolveConflictsFn = /** @type {import('./integrate.js').IntegrateFn} */ (integrate_js_1.default).resolveConflictsForMission, startAgentFn = agents_js_1.startAgent, createPrFn = forgejo_js_1.createPr, readTokenFn = forgejo_js_1.readToken, resolveForgejoUserFn = forgejo_js_1.resolveForgejoUser, resolveTaskFileFn = backlog_js_1.resolveTaskFile, getTaskImplementerFn = backlog_js_1.getTaskImplementer, detectRebaseStateFn = git_js_1.detectRebaseState, resolveMissionBaseBranchFn = mission_utils_js_1.resolveMissionBaseBranch, gitFn = git_js_1.git, exitFn = ((code) => process.exit(code)), isForgejoReviewEnabledFn = product_config_js_1.isForgejoReviewEnabled, fetchReviewBranchFn = forgejo_js_1.fetchReviewBranch, } = {}) {
    ...
    const branch = (0, mission_utils_js_1.missionBranchName)(slug);
    ...
    const performPush = async () => {
        ...
        const reviewIdentity = (0, review_state_js_1.resolveReviewIdentity)(slug, (0, mission_utils_js_1.resolveWorktree)(slug) || process.cwd());
        let forgejoUser = reviewIdentity.forgejoUser;
        if (!forgejoUser) {
            const taskResolution = resolveTaskFileFn(slug, (0, mission_utils_js_1.getPrimaryWorktree)());
        ...
        const result = createPrFn(branch, forgejoUser || 'default', token, { rootDir: (0, mission_utils_js_1.getPrimaryWorktree)(), forceWithLease: true });
        if (!result.ok) {
    ...
    if (rebaseResult.status === 0 || /up to date|Already up to date/i.test(rebaseResult.stdout + rebaseResult.stderr)) {
        ...
        if (rebaseInProg) {
            ...
            fmt.log.info(`Next: ${fmt.command((0, verification_js_1.formatVerificationCommand)(area, process.cwd()))}`);
    ...
    const rootDir = (0, mission_utils_js_1.getPrimaryWorktree)();
    const worktreePath = (0, mission_utils_js_1.resolveWorktree)(slug) || (0, mission_utils_js_1.conventionalWorktreePath)(slug, rootDir);
    ...
    if (conflictResult.ok === false && conflictResult.error === 'merge-failed') {
        ...
        const missionAbsDir = (0, mission_utils_js_1.findMissionDir)(slug, worktreePath);
        const missionDocPrefix = missionAbsDir
            ? (node_path_1.default.relative(worktreePath, missionAbsDir) + '/')
            : node_path_1.default.relative(worktreePath, (0, mission_utils_js_1.missionDirForSlug)(worktreePath, slug)).split(node_path_1.default.sep).join('/') + '/';
        const taskPattern = new RegExp(`backlog/(?:tasks|completed)/[^/]*${slug}`);
        const missionSpecificFiles = conflictFiles.filter((f) => f.startsWith(missionDocPrefix) || taskPattern.test(f));
        ...
        if (cr.error === 'worktree-missing') {
            ...
            fmt.log.info(`Ensure the worktree is registered: ${fmt.command(`git worktree add ${(0, mission_utils_js_1.conventionalWorktreePath)(slug, (0, mission_utils_js_1.getPrimaryWorktree)())} ${branch}`)}`);
            exitFn(1);
            return;
        }
...
function parseConflictFilesFromRebaseOutput(output) {
    ...
    for (const line of output.split('\n')) {
        if (!/CONFLICT|KONFLIKT/i.test(line)) {
            continue;
        }
        ...
        if (colonIdx !== -1) {
            ...
            if (!isNonPathPrefix) {
                ...
                if (/^(tips|hint|note)\b/i.test(beforeColon)) {
                    continue;
                }
                const f = beforeColon;
                if (f && !seen.has(f)) {
...
function buildRebasePrompt({ slug, area, worktreePath, missionSpecificFiles, sharedFiles, gitFn = undefined }) {
    ...
    try {
        primaryBranch = (0, mission_utils_js_1.resolveMissionBaseBranch)(slug, worktreePath, { gitFn });
    }
    catch (_) {
        try {
            primaryBranch = (0, mission_utils_js_1.getPrimaryBranch)(worktreePath, gitFn);
        }
        catch (_e) {
            primaryBranch = 'main';
        }
