...
const SHIPPED_STATS_CSV_PATH = path.join((0, package_root_js_1.packageRoot)(__dirname), 'data', 'stats.seed.csv');
function getStorage() {
    return storage;
}
...
function computeAgentMissionGroups(rows, window, options = {}) {
    ...
    if (options.completedOnly) {
        const completedMissionKeys = new Set(validWindowRows.filter(row => row.closed === 'yes').map(row => statsMissionKey(row)));
    ...
    if (options.completedOnly) {
        ...
        for (const [key, missionRows] of Object.entries(rowsByMission)) {
            ...
            const completion = completedRows.reduce((latest, row) => !latest || row.date >= latest.date ? row : latest, null);
            if (!completion) {
                continue;
            }
            const implementationModels = missionRows.filter(row => String(row.model || '').trim()
                && String(row.stage || 'default').trim().toLowerCase() !== 'review');
            const modelRow = implementationModels.reduce((latest, row) => !latest || row.date >= latest.date ? row : latest, null);
...
function classifyAgentSpendFamily(displayKey, groupRows) {
    const probe = [displayKey, ...groupRows.flatMap(row => [row.model, row.implementer, row.provider])]
        ...
        .join(' ')
        .toLowerCase();
    if (/codex|openai/.test(probe)) {
        return 'usage';
    }
    if (/claude|anthropic/.test(probe)) {
        return 'cost';
    }
    if (/mistral/.test(probe)) {
        return 'cost';
    }
    return 'duration';
}
...
function summarizeAgentStageSpend(rows, window) {
    ...
    for (const row of allValidWindowRows) {
        ...
        if (!rawRowsByDisplayKey[displayKey]) {
            rawRowsByDisplayKey[displayKey] = [];
        }
    ...
    return Object.keys(groups)
        ...
        /** @type {Record<string, number>} */
        const byStage = {};
        for (const { stage } of AGENT_SPEND_STAGE_COLUMNS) {
            byStage[stage] = 0;
        }
...
function renderWeeklyStatsReport(rows, options = {}) {
    ...
    lines.push(formatStatsTable(['Agent family', ...AGENT_SPEND_STAGE_COLUMNS.map(entry => entry.label), 'total'], currentAgentSpend.length > 0
