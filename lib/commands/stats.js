...
function summarizeMissionWindow(rows, window) {
  ...
  const unknown = uniqueMissions.filter(row => row.classification === 'unknown').length;
  return {
    rows: windowRows,
...
function resolveMissionClassification(slug, rootDir = process.cwd()) {
  ...
  if (!classification) {
    throw new Error(`Missing or invalid classification for ${slug}; expected exactly one of ai_sdlc, user_value, or unknown in the labels of ${resolution.taskFile}. Fix: add exactly one of those labels and do not use a separate frontmatter field for mission type.`);
  }
