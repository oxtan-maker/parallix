...
function cleanupNewTempDirs(beforeEntries, tmpRoot = os.tmpdir()) {
    ...
    for (const name of fs.readdirSync(tmpRoot)) {
        if (beforeEntries.has(name) || !shouldCleanTempDir(name)) {
            continue;
        }
...
function runTests(testFiles, coverageThreshold = threshold, _spawnSync = node_child_process_1.spawnSync) {
...
run.buildCoverageArgs = buildCoverageArgs;
run.cleanupNewTempDirs = cleanupNewTempDirs;
run.cleanupPerRunScratch = cleanupPerRunScratch;
run.createPerRunScratchDirs = createPerRunScratchDirs;
run.COVERAGE_EXCLUDES = COVERAGE_EXCLUDES;
run.COVERAGE_INCLUDES = COVERAGE_INCLUDES;
run.DEFAULT_TEST_TIMEOUT_MS = DEFAULT_TEST_TIMEOUT_MS;
run.discoverTestFiles = discoverTestFiles;
run.listTempEntries = listTempEntries;
run.registerExitHandlers = registerExitHandlers;
run.resetPerRunScratchState = resetPerRunScratchState;
