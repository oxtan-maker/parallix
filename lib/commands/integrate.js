...
function printIntegrationPreflight(
  context,
  {
    ...
    getPrimaryWorktreeFn = getPrimaryWorktree,
    log = fmt.log.plain
  } = {}
) {
  const failures = [];
  ...
  const baseWorktree = context.baseWorktree || getPrimaryWorktreeFn();
  const baseBranch = context.baseBranch || getPrimaryBranchFn();
