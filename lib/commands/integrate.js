...
function maybeUpdateGraphifyOnPrimary(rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), opts = {}) {
    return (0, mission_utils_js_1.updateGraphifyKnowledgeGraph)({
        ...
        log: opts.log,
        startMessage: `Updating graphify knowledge graph on ${(0, mission_utils_js_1.getPrimaryBranch)()}...`,
...
const INTEGRATION_CONFIG_PATH = node_path_1.default.join((0, mission_utils_js_1.getPrimaryWorktree)(), 'config', 'integration-pipelines.json');
/**
...
async function integrate(args) {
    ...
    try {
        ...
        if (!noIntegrationGates) {
            ...
            const result = node_child_process_1.default.spawnSync('bash', ['-lc', verification.command], {
...
function buildIntegrationContext(slug, { baseBranch = null, baseWorktree = null, isForgejoReviewEnabledFn = product_config_js_1.isForgejoReviewEnabled } = {}) {
    ...
    let task = (0, backlog_js_1.resolveTaskFile)(slug);
    if (!task.ok) {
        const worktree = (0, mission_utils_js_1.resolveWorktree)(slug);
        if (worktree) {
            task = (0, backlog_js_1.resolveTaskFile)(slug, worktree);
        }
    }
    let taskStatus = task.ok ? (0, backlog_js_1.getTaskStatus)(task.taskFile) : null;
    if (task.ok && taskStatus === 'backlog') {
        const worktree = (0, mission_utils_js_1.resolveWorktree)(slug);
        if (worktree) {
            const wtTask = (0, backlog_js_1.resolveTaskFile)(slug, worktree);
            if (wtTask.ok) {
                const wtStatus = (0, backlog_js_1.getTaskStatus)(wtTask.taskFile);
    ...
    if (forgejoEnabled) {
        ...
        forgejoToken = (0, forgejo_js_1.readToken)(/** @type {any} */ (forgejoIdentity.forgejoUser || 'default'));
        pr = /** @type {any} */ ((0, forgejo_js_1.getPrStatus)(branch, process.cwd(), {
        ...
        approval = pr.exists ? /** @type {any} */ ((0, forgejo_js_1.getLatestReviewDecision)(branch, {
            forgejoUser: /** @type {any} */ (forgejoIdentity.forgejoUser),
    ...
    if (forgejoEnabled && !approval.ok) {
        const localStateFallback = (0, review_state_js_1.readReviewState)(slug, /** @type {string} */ (resolvedBaseWorktree));
    ...
    const mainBranchResult = (0, git_js_1.git)(['-C', /** @type {string} */ (resolvedBaseWorktree), 'branch', '--show-current']);
    const mainBranch = mainBranchResult.stdout.trim();
    const mainStatus = (0, git_js_1.git)(['-C', /** @type {string} */ (resolvedBaseWorktree), 'status', '--short']);
    return {
        slug,
        branch,
        currentBranch,
...
function evaluateTaskStatusForIntegration(context) {
    ...
    return {
        ok: false,
        level: 'fail',
        message: `Backlog status: expected approved, or review with an approved/merged Forgejo PR; found ${(0, state_map_js_1.toVirtual)(context.taskStatus, stateMapOptions)}`
    };
}
/** @param {{slug: string, branch: string, currentBranch: string, missionDir?: string, area: string, task: {ok: boolean, taskFile?: string, reason?: string, matches?: string[]}, taskStatus?: string, taskAssignee?: string|null, forgejoUser?: string|null, forgejoToken?: string|null, taskAssigneeWarning?: string|null, pr: {exists?: boolean, state?: string, number?: number, merged?: boolean, raw?: string}, siblingPrs: any[], approval: {ok?: boolean, error?: string, reviewState?: string, defaultUserApproved?: boolean, source?: string}, baseBranch?: string, baseWorktree?: string, mainBranch: string, mainDirtyEntries: string[], mainDirty: boolean}} context */
function promoteTaskForIntegrationIfNeeded(context, { dryRun = false } = {}) {
    ...
    if (!(0, backlog_js_1.setTaskStatus)(/** @type {string} */ (context.task.taskFile || ''), approvedStatus)) {
...
function printIntegrationPreflight(context, { readTokenFn = forgejo_js_1.readToken, resolveTokenFileFn = forgejo_js_1.resolveTokenFile, detectRebaseStateFn = git_js_1.detectRebaseState, getUnresolvedIndexConflictsFn = getUnresolvedIndexConflicts, findMissionDocInBranchesFn = mission_utils_js_1.findMissionDocInBranches, isForgejoReviewEnabledFn = product_config_js_1.isForgejoReviewEnabled, log = fmt.log.plain } = {}) {
    ...
    if (context.task.ok) {
        ...
        try {
            const { classification, error: classificationError } = stats_js_1.default.resolveMissionClassification(context.slug);
            if (!classification) {
 ...
 */
function recordPostIntegrationStats(slug, { rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), gitRunner = git_js_1.git, recordIntegrationStatsFn = stats_js_1.default.recordIntegrationStats, } = {}) {
...
function finalizeVariantACloseout({ slug, summary, mainTaskFile, rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), gitRunner = git_js_1.git, baseBranch = null, verificationArea = null, captureVerifiedTreeProofFn = verification.captureVerifiedTreeProof, assertVerifiedTreeProofFn = verification.assertVerifiedTreeProof }) {
    (0, mission_utils_js_1.softResetTrailingBacklogNoise)(rootDir, gitRunner);
    if (mainTaskFile && node_fs_1.default.existsSync(mainTaskFile)) {
        (0, backlog_js_1.completeTask)(slug, rootDir);
        // Re-resolve because it moved
        const updatedResolution = (0, backlog_js_1.resolveTaskFile)(slug, rootDir);
        if (updatedResolution.ok) {
    ...
    const commitResult = gitRunner([
        ...
        'commit',
        '-m',
        `${(0, mission_utils_js_1.missionBranchName)(slug, rootDir)}: ${summary} integration closeout`
    ]);
    if (commitResult.status !== 0) {
        return {
            ok: false,
    ...
    const pushTarget = baseBranch || (0, mission_utils_js_1.getPrimaryBranch)(gitRunner);
...
integrate.finalizeVariantACloseout = finalizeVariantACloseout;
