...
exports.parseIntegrateArgs = parseIntegrateArgs;
...
exports.runPostIntegrateHookOrAbort = runPostIntegrateHookOrAbort;
exports.refreshBuildBeforeVerification = refreshBuildBeforeVerification;
...
const postIntegrateHook = __importStar(require("../core/post-integrate-hook.js"));
const build_freshness_js_1 = require("../core/build-freshness.js");
...
function maybeUpdateGraphifyOnPrimary(rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), opts = {}) {
    return (0, mission_utils_js_1.updateGraphifyKnowledgeGraph)({
        rootDir,
        commandRunner: opts.commandRunner || ((/** @type{string} */ command, /** @type{string[]} */ args, /** @type{object} */ options) => (0, git_js_1.run)(command, args, options)),
        log: opts.log,
        startMessage: `Updating graphify knowledge graph on ${(0, mission_utils_js_1.getPrimaryBranch)(rootDir)}...`,
...
function prepareNoisePatchForSquash(rootDir, opts = {}) {
    const runner = (opts.gitRunner || git_js_1.git);
    ...
    const patchDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'parallix-integrate-noise-'));
    const patchPath = node_path_1.default.join(patchDir, 'backlog-noise.patch');
    node_fs_1.default.writeFileSync(patchPath, patch, 'utf8');
    ...
    return {
        ok: true,
        patchPath,
        cleanup: () => node_fs_1.default.rmSync(patchDir, { recursive: true, force: true })
    };
}
...
function resolveForgejoUserForIntegration(taskAssignee) {
    ...
    return {
        forgejoUser: (0, forgejo_js_1.resolveForgejoUser)(""),
        warning: null
    };
}
/** @param {string} rootDir @param {{gitRunner?: Function}} opts */
function getUnresolvedIndexConflicts(rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), opts = {}) {
    const runner = (opts.gitRunner || git_js_1.git);
    ...
    const files = Array.from(new Set(result.stdout
        .split('\n')
        .map((line) => line.trim())
        .filter(Boolean)
...
function reportStashPopFailure(slug, restoreResult, opts = {}) {
    ...
    const runner = (opts.gitRunner || git_js_1.git);
    const rootDir = opts.rootDir || (0, mission_utils_js_1.getPrimaryWorktree)();
    const headResult = runner(['-C', rootDir, 'log', '-1', '--oneline']);
    const headLine = headResult.status === 0 ? headResult.stdout.trim() : '(unavailable)';
    const integrationLanded = headLine.includes(`${(0, mission_utils_js_1.missionBranchName)(slug, rootDir)}:`);
    ...
    if (indexConflicts.ok && indexConflicts.files.length > 0) {
        ...
        indexConflicts.files.forEach((file) => fmt.log.fail(`  Collision file: ${file}`));
        ...
        if (collisionFiles.length > 0) {
            collisionFiles.forEach((file) => fmt.log.fail(`  Collision file: ${file}`));
        }
        fmt.log.fail('  Recovery steps:');
        fmt.log.fail(`    1. git -C ${rootDir} stash show --name-only stash@{0}`);
        if (collisionFiles.length > 0) {
            collisionFiles.forEach((file) => {
                fmt.log.fail(`    2. mv ${node_path_1.default.join(rootDir, file)} ${node_path_1.default.join(rootDir, `${file}.pre-stash-pop`)}`);
            });
    ...
    if (output) {
        ...
        output.split('\n').forEach((line) => fmt.log.fail(`    ${line}`));
    }
}
...
function getIntegrationConfigPath() {
    return node_path_1.default.join((0, mission_utils_js_1.getPrimaryWorktree)(), 'config', 'integration-pipelines.json');
}
/**
...
function detectChangedAreas(slug, opts = {}) {
    const primaryBranch = opts.baseBranch || (0, mission_utils_js_1.getPrimaryBranch)();
    const branch = `mission/${slug}`;
    const runner = (opts.gitRunner || git_js_1.git);
    // Get changed files between primary branch and mission branch
    const diffResult = runner(['-C', opts.rootDir || (0, mission_utils_js_1.getPrimaryWorktree)(), 'diff', '--name-only', primaryBranch, branch, '--']);
    if (diffResult.status !== 0 && !diffResult.stdout.trim()) {
        // No changes or error - fall back to checking against HEAD in the worktree
        const worktree = (0, mission_utils_js_1.resolveWorktree)(slug) || (0, mission_utils_js_1.conventionalWorktreePath)(slug, opts.rootDir);
        if (node_fs_1.default.existsSync(worktree)) {
...
function gateMatchesChangedAreas(gateKey, changedAreas, gateAreas) {
...
function loadIntegrationConfig(opts = {}) {
    ...
    if (!node_fs_1.default.existsSync(configPath) || !node_fs_1.default.statSync(configPath).size) {
...
function buildIntegrationGateEnv(slug, opts = {}) {
    ...
    if (opts.realAgent && opts.realAgentModel) {
        env.PARALLIX_REAL_AGENT = opts.realAgent;
        env.PARALLIX_REAL_AGENT_MODEL = opts.realAgentModel;
    }
    return env;
}
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (INTEGRATE_VALUE_OPTIONS.has(arg)) {
            ...
            if (arg === REAL_AGENT_OPTION) {
                if (realAgent !== null) {
                    throw new Error('--real-agent may be supplied only once.');
                }
                realAgent = value;
            }
            else {
                if (realAgentModel !== null) {
                    throw new Error('--real-agent-model may be supplied only once.');
                }
        ...
        if (arg === '--no-integration-gates') {
            noIntegrationGates = true;
            continue;
        }
        if (arg === '--no-gate') {
            noGate = true;
        ...
        if (arg.startsWith('--')) {
            throw new Error(`Unknown integrate option: ${arg}`);
        }
        params.push(arg);
    }
...
function resolveIntegrationVerificationWorktree(slug, opts = {}) {
    const resolveFn = (opts.resolveWorktreeFn || mission_utils_js_1.resolveWorktree);
    const convFn = (opts.conventionalWorktreePathFn || mission_utils_js_1.conventionalWorktreePath);
...
async function executeIntegrationGates(gates, opts = {}) {
    const runner = (opts.commandRunner || ((/** @type{string} */ cmd) => node_child_process_1.default.spawnSync(cmd, {
        shell: true,
        cwd: opts.rootDir || (0, mission_utils_js_1.getPrimaryWorktree)(),
        stdio: 'inherit'
    })));
    for (const gate of gates) {
...
async function integrate(args) {
    ...
    try {
        ...
        if (!noIntegrationGates) {
            ...
            const result = node_child_process_1.default.spawnSync('bash', ['-c', verification.command], {
        ...
        const missionWorktree = (0, mission_utils_js_1.conventionalWorktreePath)(slug);
        ...
        const branch = (0, mission_utils_js_1.missionBranchName)(slug, baseWorktree);
        const mainTitle = (0, mission_utils_js_1.missionTitle)(slug) || slug;
        ...
        const dryMerge = (0, git_js_1.git)(['-C', baseWorktree, 'merge', '--no-commit', '--no-ff', branch]);
        const abortResult = (0, git_js_1.git)(['-C', baseWorktree, 'merge', '--abort']);
        ...
        if (dryMerge.status !== 0) {
            ...
            if (existingSquash) {
                ...
                if ((0, product_config_js_1.isForgejoReviewEnabled)(baseWorktree)) {
                    fmt.log.info('Step 6 (resume): Syncing merged state to Forgejo...');
                    const syncResult = (0, forgejo_js_1.syncMerged)(branch, mergedCommit, {
                ...
                if (node_fs_1.default.existsSync(baseWorktree)) {
                ...
                runPostIntegrateHookOrAbort(slug, { baseWorktree: baseWorktree, baseBranch: baseBranch, variant: 'variant-b-resumed' });
                ...
                const conflictFiles = (0, mission_utils_js_1.parseConflictFilesFromMergeOutput)(conflictOutput);
                ...
                for (const line of (0, runtime_matrix_js_1.formatMatrixSummary)((0, runtime_matrix_js_1.buildAutonomousReviewMatrix)())) {
                    fmt.log.info(line);
                }
            ...
            if ((0, mission_utils_js_1.softResetTrailingBacklogNoise)(baseWorktree, git_js_1.git)) {
                noisePatchState = prepareNoisePatchForSquash(baseWorktree, { gitRunner: git_js_1.git });
            ...
            const squashResult = (0, git_js_1.git)(['-C', baseWorktree, 'merge', '--squash', branch]);
            ...
            if (noisePatchState?.patchPath) {
                const restoreNoiseResult = restoreNoisePatchAfterSquash(/** @type {string} */ (baseWorktree), noisePatchState.patchPath, { gitRunner: git_js_1.git });
                noisePatchState.cleanup();
            ...
            if (node_fs_1.default.existsSync(mainTaskFile)) {
                (0, backlog_js_1.completeTask)(slug, baseWorktree);
                // Re-resolve because it moved
                const updatedResolution = (0, backlog_js_1.resolveTaskFile)(slug, baseWorktree);
                if (updatedResolution.ok) {
                    rewriteWorktreePaths(updatedResolution.taskFile, slug, { rootDir: baseWorktree });
                }
            }
            (0, git_js_1.git)(['-C', baseWorktree, 'add', '-A']);
            fmt.log.info('Step 5: Creating the landed squash commit in the local integration checkout...');
            const commitResult = (0, git_js_1.git)([
                '-C',
            ...
            const refreshResult = refreshBuildBeforeVerification(baseWorktree, {
                runFn: node_child_process_1.default.spawnSync
            });
            if (!refreshResult.ok) {
            ...
            const mergedCommit = (0, git_js_1.git)(['-C', baseWorktree, 'rev-parse', 'HEAD']).stdout.trim();
            if ((0, product_config_js_1.isForgejoReviewEnabled)(baseWorktree)) {
            ...
            maybeUpdateGraphifyOnPrimary(baseWorktree);
            runPostIntegrateHookOrAbort(slug, { baseWorktree: baseWorktree, baseBranch: baseBranch, variant: 'variant-b' });
            ...
            const proofResult = verification.captureVerifiedTreeProof(context.area, baseWorktree, {
                gitRunner: git_js_1.git,
                runFn: /** @type {Function} */ (node_child_process_1.default.spawnSync)
            });
            if (!(proofResult).ok) {
            ...
            const proofCheck = verification.assertVerifiedTreeProof(proof, baseWorktree, { gitRunner: git_js_1.git });
            if (!proofCheck.ok) {
...
function buildIntegrationContext(slug, { baseBranch = null, baseWorktree = null, isForgejoReviewEnabledFn = product_config_js_1.isForgejoReviewEnabled, getCurrentBranchFn = git_js_1.getCurrentBranch, readTokenFn = forgejo_js_1.readToken, getPrStatusFn = forgejo_js_1.getPrStatus, getLatestReviewDecisionFn = forgejo_js_1.getLatestReviewDecision, readReviewStateFn = review_state_js_1.readReviewState, gitFn = git_js_1.git } = {}) {
    if (!slug) {
    ...
    if (!resolvedBaseBranch) {
        try {
            resolvedBaseBranch = (0, mission_utils_js_1.resolveMissionBaseBranch)(slug, process.cwd());
        }
        catch (_) {
            resolvedBaseBranch = (0, mission_utils_js_1.getPrimaryBranch)();
        }
    }
    let resolvedBaseWorktree = baseWorktree;
    if (!resolvedBaseWorktree) {
        try {
            resolvedBaseWorktree = (0, mission_utils_js_1.resolveBaseWorktree)(slug, { rootDir: process.cwd() });
        }
        catch (_) {
            resolvedBaseWorktree = (0, mission_utils_js_1.getPrimaryWorktree)();
        }
    }
    ...
    // review approval.
    const worktree = (0, mission_utils_js_1.resolveWorktree)(slug);
    /** @type {ReturnType<typeof resolveTaskFile>} */
    const baseTask = (0, backlog_js_1.resolveTaskFile)(slug, /** @type {string} */ (resolvedBaseWorktree));
    let task = worktree ? (0, backlog_js_1.resolveTaskFile)(slug, worktree) : { ok: false, reason: 'missing', matches: [] };
    if (!task.ok) {
        task = baseTask;
    }
    const taskStatusSource = baseTask.ok ? baseTask : task;
    const taskStatus = taskStatusSource.ok ? (0, backlog_js_1.getTaskStatus)(taskStatusSource.taskFile) : null;
    const taskAssignee = task.ok ? (0, backlog_js_1.getTaskAssignee)(task.taskFile) : null;
    ...
    if (forgejoEnabled) {
        ...
        if (pr.exists && slug) {
            ...
            if (forgejoToken) {
                const allOpen = (0, forgejo_js_1.listOpenPrsForSlug)(baseSlug, forgejoToken);
...
function evaluateTaskStatusForIntegration(context) {
    ...
    if ((0, state_map_js_1.toVirtual)(context.taskStatus, /** @type {any} */ (stateMapOptions)) === 'approved') {
        return {
            ok: true,
            level: 'pass',
    ...
    return {
        ok: false,
        level: 'fail',
        message: `Backlog status: expected approved, or review with an approved Forgejo PR; found ${(0, state_map_js_1.toVirtual)(context.taskStatus, stateMapOptions)}`
    };
}
/** @param {Function} log @param {string} slug @param {string} baseWorktree @param {string} baseBranch */
function printMergedPrRecoveryGuidance(log, slug, baseWorktree, baseBranch) {
...
function promoteTaskForIntegrationIfNeeded(context, { dryRun = false } = {}) {
    ...
    const stateMapOptions = { rootDir: /** @type {string} */ (context.baseWorktree) };
    const approvedStatus = (0, state_map_js_1.toActual)('approved', stateMapOptions) || 'approved';
    const baseTask = context.slug && context.baseWorktree
        ? (0, backlog_js_1.resolveTaskFile)(context.slug, context.baseWorktree)
        : context.task;
    if (!baseTask?.ok || !(0, backlog_js_1.setTaskStatus)(/** @type {string} */ (baseTask.taskFile || ''), approvedStatus)) {
        fmt.log.fail('Could not promote the Backlog task to approved before integration.');
        throw new IntegrationAbort();
    }
    context.taskStatus = (0, state_map_js_1.toActual)('approved', stateMapOptions);
 ...
 */
function printIntegrationPreflight(context, { readTokenFn = forgejo_js_1.readToken, resolveTokenFileFn = forgejo_js_1.resolveTokenFile, detectRebaseStateFn = git_js_1.detectRebaseState, getUnresolvedIndexConflictsFn = getUnresolvedIndexConflicts, findMissionDocInBranchesFn = mission_utils_js_1.findMissionDocInBranches, isForgejoReviewEnabledFn = product_config_js_1.isForgejoReviewEnabled, log = fmt.log.plain } = {}) {
    ...
    const baseWorktree = context.baseWorktree || (0, mission_utils_js_1.getPrimaryWorktree)();
    const baseBranch = context.baseBranch || (0, mission_utils_js_1.getPrimaryBranch)();
    log(fmt.status('INFO', `Integration preflight for ${context.slug}`));
    const branchPrefix = (0, mission_utils_js_1.missionBranchName)(context.slug, baseWorktree);
    ...
    if (context.missionDir) {
        log(fmt.status('PASS', `Mission doc: ${node_path_1.default.join(context.missionDir, 'MISSION.md')}`));
    }
    else {
        ...
        const canonicalPath = node_path_1.default.relative(baseWorktree, node_path_1.default.join((0, mission_utils_js_1.missionDirForSlug)(baseWorktree, canonicalSlug), 'MISSION.md')).split(node_path_1.default.sep).join('/');
    ...
    if (context.task.ok) {
        log(fmt.status('PASS', `Backlog task: ${node_path_1.default.basename(/** @type {string} */ (context.task.taskFile))} (${context.taskStatus})`));
        try {
            const classification = (0, backlog_js_1.getTaskClassification)(/** @type {string} */ (context.task.taskFile));
            ...
            failures.push('classification');
            log(fmt.status('FAIL', `Backlog classification: ${error.message || String(error)}`));
        }
        const taskStatusCheck = evaluateTaskStatusForIntegration(context);
    ...
    if (context.mainDirty) {
        ...
        const overlapPaths = [];
        if (context.missionDir) {
            const relMissionPath = node_path_1.default.relative(baseWorktree, context.missionDir);
            overlapPaths.push(relMissionPath);
        }
        overlapPaths.push(`missions/${context.slug}`);
        const overlappingEntries = [];
        const nonOverlappingEntries = [];
        context.mainDirtyEntries.forEach((entry) => {
            ...
            if (!isOverlap) {
                for (const mp of overlapPaths) {
                    if (mp && (filePath === mp || filePath.startsWith(mp + node_path_1.default.sep) || filePath.startsWith(mp + '/'))) {
                        isOverlap = true;
                        break;
                    }
                }
        ...
        if (overlappingEntries.length > 0) {
            ...
            overlappingEntries.forEach((entry) => log(fmt.status('WARN', `  - ${entry}`)));
            fmt.log.fail('Recovery steps:');
            ...
            nonOverlappingEntries.forEach((entry) => log(fmt.status('INFO', `  - ${entry}`)));
        }
        else {
    ...
    const gitDir = node_path_1.default.join(process.cwd(), '.git');
    const isMainRepo = node_fs_1.default.existsSync(gitDir) && !node_fs_1.default.lstatSync(gitDir).isSymbolicLink();
...
function rewriteWorktreePaths(taskFilePath, slug, options = {}) {
    const rootDir = options.rootDir || (0, mission_utils_js_1.getPrimaryWorktree)();
    const before = node_fs_1.default.readFileSync(taskFilePath, 'utf8');
    const updated = before.replace(new RegExp(`${(0, mission_utils_js_1.conventionalWorktreePath)(slug, rootDir)}`, 'g'), rootDir);
    if (before !== updated) {
        node_fs_1.default.writeFileSync(taskFilePath, updated, 'utf8');
    }
}
/** @param{{slug: string, dirtyEntries?: string[], rootDir?: string, gitRunner?: Function}} params */
function stashMainCheckoutIfNeeded({ slug, dirtyEntries = [], rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), gitRunner = git_js_1.git }) {
...
function restoreMainCheckoutStash({ message, rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), gitRunner = git_js_1.git }) {
 ...
 */
function recordPostIntegrationStats(slug, { rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), recordIntegrationStatsFn = stats_js_1.default.recordIntegrationStats, } = {}) {
    ...
    // timestamp.
    const statsCsvPath = stats_js_1.default.resolveStatsFilePath(rootDir);
    ...
    const missionReport = stats_js_1.default.renderMissionPhaseReport(missionRows, slug);
 ...
 */
function runPostIntegrateHookOrAbort(slug, { baseWorktree, baseBranch, variant, runPostIntegrateHookFn = postIntegrateHook.runPostIntegrateHook }) {
 ...
 */
function refreshBuildBeforeVerification(rootDir, { runFn = node_child_process_1.default.spawnSync, log = fmt.log.info } = {}) {
    ...
    const refreshedFreshness = (0, build_freshness_js_1.getBuildFreshnessStatus)(rootDir);
 ...
 */
function cleanupMissionWorktree(slug, { rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), gitRunner = git_js_1.git, removeDir = (target) => {
    if ((0, forgejo_js_1.isForgejoPath)(target, { forgejoHome: (0, forgejo_js_1.resolveForgejoHome)() })) {
        throw new Error(`CRITICAL SAFETY VIOLATION: cleanupMissionWorktree attempted to delete Forgejo home: ${target}`);
    }
    return node_fs_1.default.rmSync(target, { recursive: true, force: true });
}, existsSync = node_fs_1.default.existsSync } = {}) {
    const branch = (0, mission_utils_js_1.missionBranchName)(slug, rootDir);
    const worktreePath = `${(0, mission_utils_js_1.conventionalWorktreePath)(slug, rootDir)}`;
...
function findExistingSquashCommit(rootDir, slug) {
    const result = (0, git_js_1.git)(['-C', rootDir, 'log', '--format=%H %s', '-50']);
    if (result.status !== 0) {
        return null;
    }
    const prefix = `${(0, mission_utils_js_1.missionBranchName)(slug, rootDir)}:`;
...
function resolveConflictsForMission(slug, area, { getConflictFilesFn, resolveWorktreeFn = mission_utils_js_1.resolveWorktree, worktreePathOverride = null, rootDir = (0, mission_utils_js_1.getPrimaryWorktree)(), baseBranch = null } = {}) {
    ...
    const conventionalPath = `${(0, mission_utils_js_1.conventionalWorktreePath)(slug, rootDir)}`;
    const resolvedPath = worktreePathOverride || resolveWorktreeFn(slug);
    const worktreePath = resolvedPath || conventionalPath;
    if (!resolvedPath) {
        fmt.log.warn(`No registered worktree found for ${(0, mission_utils_js_1.missionBranchName)(slug, rootDir)}; falling back to conventional path.`);
    }
    ...
    /** @type {string} */
    let targetBranch = /** @type {string} */ (baseBranch || (0, mission_utils_js_1.getPrimaryBranch)());
    if (!targetBranch) {
        try {
            targetBranch = (0, mission_utils_js_1.resolveMissionBaseBranch)(slug, rootDir);
        }
        catch (_) {
            targetBranch = (0, mission_utils_js_1.getPrimaryBranch)();
        }
    }
    const detectFn = getConflictFilesFn || (() => (0, mission_utils_js_1.getConflictFiles)(worktreePath, targetBranch));
    if (!node_fs_1.default.existsSync(worktreePath)) {
    ...
    const missionSpecificFiles = conflictFiles.filter((f) => (0, mission_utils_js_1.isMissionArtifact)(f, slug, worktreePath) || taskPattern.test(f));
...
function buildConflictResolutionPrompt(slug = '<slug>', area = '<area>', options = {}) {
    const rootDir = options.rootDir || (0, mission_utils_js_1.getPrimaryWorktree)();
    const worktreePath = options.worktreePath || (0, mission_utils_js_1.resolveWorktree)(slug) || (0, mission_utils_js_1.conventionalWorktreePath)(slug, rootDir);
    const quotedWorktreePath = shellQuote(worktreePath);
...
integrate.resolveConflictsForMission = resolveConflictsForMission;
integrate.cleanupMissionWorktree = cleanupMissionWorktree;
integrate.rewriteWorktreePaths = rewriteWorktreePaths;
integrate.isNoMergeToAbortResult = isNoMergeToAbortResult;
integrate.buildConflictResolutionPrompt = buildConflictResolutionPrompt;
integrate.VARIANT_B_AUTOMATION_SUMMARY = VARIANT_B_AUTOMATION_SUMMARY;
integrate.stashMainCheckoutIfNeeded = stashMainCheckoutIfNeeded;
integrate.restoreMainCheckoutStash = restoreMainCheckoutStash;
integrate.evaluateTaskStatusForIntegration = evaluateTaskStatusForIntegration;
integrate.promoteTaskForIntegrationIfNeeded = promoteTaskForIntegrationIfNeeded;
integrate.findExistingSquashCommit = findExistingSquashCommit;
integrate.printIntegrationPreflight = printIntegrationPreflight;
integrate.resolveForgejoUserForIntegration = resolveForgejoUserForIntegration;
integrate.getUnresolvedIndexConflicts = getUnresolvedIndexConflicts;
integrate.parseStashPopCollisionFiles = parseStashPopCollisionFiles;
integrate.reportStashPopFailure = reportStashPopFailure;
integrate.maybeUpdateGraphifyOnPrimary = maybeUpdateGraphifyOnPrimary;
integrate.SYNC_MERGED_DIAGNOSTICS = SYNC_MERGED_DIAGNOSTICS;
integrate.printDiagnosticTable = printDiagnosticTable;
integrate.reportSyncMergedFailure = reportSyncMergedFailure;
integrate.recordPostIntegrationStats = recordPostIntegrationStats;
integrate.recordPostIntegrationStatsOrAbort = recordPostIntegrationStatsOrAbort;
integrate.runPostIntegrateHookOrAbort = runPostIntegrateHookOrAbort;
integrate.refreshBuildBeforeVerification = refreshBuildBeforeVerification;
integrate.formatRecordedStatsRow = formatRecordedStatsRow;
integrate.detectChangedAreas = detectChangedAreas;
integrate.parseFilesToAreas = parseFilesToAreas;
integrate.loadIntegrationConfig = loadIntegrationConfig;
integrate.getIntegrationGatePlan = getIntegrationGatePlan;
integrate.printIntegrationGatePlan = printIntegrationGatePlan;
integrate.buildIntegrationGateEnv = buildIntegrationGateEnv;
integrate.parseIntegrateArgs = parseIntegrateArgs;
