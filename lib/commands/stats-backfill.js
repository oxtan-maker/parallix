...
function listHistoricalMissionSlugs(rootDir = process.cwd()) {
    ...
    for (const year of fs.readdirSync(missionsRoot).sort()) {
        ...
        for (const slug of fs.readdirSync(yearDir).sort()) {
            if (!/^task-\d+/i.test(slug)) {
                continue;
            }
            const missionDir = path.join(yearDir, slug);
            if (!fs.statSync(missionDir).isDirectory()) {
                continue;
            }
            if (!hasCheckpointFiles(missionDir)) {
                continue;
            }
            slugs.push(slug);
        }
...
function normalizeHistoricalImplementer(value) {
    ...
    if (/(^|[^a-z])codex([^a-z]|$)/.test(normalized)) {
        return 'codex';
    }
    if (/(^|[^a-z])claude([^a-z]|$)/.test(normalized)) {
        return 'claude';
    }
    if (/(^|[^a-z])gemini([^a-z]|$)/.test(normalized)) {
        return 'gemini';
    }
    if (/(^|[^a-z])custom([^a-z]|$)/.test(normalized)) {
        return 'custom';
    }
    if (/(^|[^a-z])vibe([^a-z]|$)/.test(normalized)) {
        return 'vibe';
    }
    if (/(^|[^a-z])magnus([^a-z]|$)/.test(normalized)) {
        return 'magnus';
    }
    if (/(^|[^a-z])human([^a-z]|$)/.test(normalized)) {
        return 'human';
    }
    return null;
}
function deriveImplementerFromGitHistory(slug, taskFile, rootDir = process.cwd()) {
    ...
    const targets = [missionDir, taskFile].filter((x) => Boolean(x));
    if (targets.length === 0) {
        return null;
    }
    const result = (0, git_js_1.git)(['-C', rootDir, 'log', '--all', '--format=%an|%ae', '--', ...targets]);
    if (result.status !== 0) {
        return null;
    }
...
function deriveDateFromGitHistory(slug, taskFile, rootDir = process.cwd()) {
    ...
    const result = (0, git_js_1.git)(['-C', rootDir, 'log', '--all', '-1', '--date=short', '--format=%cd', '--', ...targets]);
    if (result.status !== 0) {
        return null;
    }
...
function inferHistoricalClassificationFromMissionDoc(slug, rootDir = process.cwd()) {
    ...
    if (productScore === 0 && workflowScore === 0) {
        return null;
    }
    if (workflowScore >= productScore + 2) {
        return 'ai_sdlc';
    }
    if (productScore >= workflowScore + 2) {
        return 'user_value';
    }
...
function resolveHistoricalClassification(slug, taskFile, rootDir = process.cwd()) {
    ...
    const classificationValue = (0, backlog_js_1.getTaskFrontmatterValue)(taskFile, 'classification');
    const legacy = s._internals.normalizeClassification(classificationValue ?? '');
    if (legacy) {
...
function collectHistoricalStatsBackfill(rootDir = process.cwd(), filePath = null) {
    ...
    const existingMissions = new Set(s.loadStatsCsv(filePath, { rootDir }).rows
    ...
    for (const slug of listHistoricalMissionSlugs(rootDir)) {
        if (existingMissions.has(slug)) {
            continue;
        }
        const taskResolution = (0, backlog_js_1.resolveTaskFile)(slug, rootDir);
        ...
        const status = (0, backlog_js_1.getTaskStatus)(taskFile);
        if (status !== 'done') {
            skipped.push({ slug, reason: `status=${status || 'unknown'}` });
            continue;
        }
        const date = extractDateOnly((0, backlog_js_1.getTaskFrontmatterValue)(taskFile, 'updated_date') ?? '') || deriveDateFromGitHistory(slug, taskFile, rootDir);
