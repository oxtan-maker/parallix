...
function runDeclaredGates(missionDir, rootDir, options = {}) {
  ...
  // Extract command between backticks if present, otherwise use entire line
  .map(cmd => {
    const backtickMatch = cmd.match(/^`(.+?)`/);
    return backtickMatch ? backtickMatch[1] : cmd;
  });
  ...
  for (const cmd of commands) {
    log(`  Gate: ${cmd}`);
    let result = spawnSync('bash', ['-c', cmd], {
      cwd: rootDir,
      encoding: 'utf8',
    ...
    if (result.status !== 0) {
      ...
      // Treat `grep -c` returning 1 with all counts being 0 as success (no matches found)
      if (cmd.startsWith('grep -c') && result.status === 1) {
        const allZeros = stdout.split('\n').every(line => {
          const parts = line.split(':');
          return parts.length === 2 && parts[1].trim() === '0';
        });
        if (allZeros && stdout.trim().length > 0) {
          log(`  Gate: grep -c found 0 matches in all files (expected)`);
          continue;
        }
      }
      // Retry with npx prepended if command not found (e.g., `tsc` -> `npx tsc`)
      // Skip npx retry for known system commands that don't need npm
      if ((stderr.includes('kommandot finns inte') || stderr.includes('command not found'))
          && !cmd.startsWith('git ')
          && !cmd.startsWith('npm ')
          && !cmd.startsWith('node ')) {
        log(`  Gate: retrying with npx: npx ${cmd}`);
        result = spawnSync('bash', ['-c', `npx ${cmd}`], {
          cwd: rootDir,
          encoding: 'utf8',
          stdio: ['inherit', 'pipe', 'pipe']
        });
      ...
      if (result.status !== 0) {
        const retryStderr = (result.stderr || '').trim();
        return {
          ok: false,
          gate: cmd,
          reason: 'gate-failed',
          error: retryStderr || stderr || `Gate exited with status ${result.status}`
        };
      }
    }
  }
