...
function runDeclaredGates(missionDir, rootDir, options = {}) {
  ...
  for (const cmd of commands) {
    ...
    if (result.status !== 0) {
      ...
      // Treat `grep -c` returning 1 with stdout "0" as success (no matches found)
      if (cmd.startsWith('grep -c') && result.status === 1 && stdout === '0') {
        log(`  Gate: grep -c found 0 matches (expected)`);
        continue;
      }
