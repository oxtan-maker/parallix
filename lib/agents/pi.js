...
exports.buildPiInvocation = buildPiInvocation;
exports.extractPiSessionId = extractPiSessionId;
exports.extractPiTelemetry = extractPiTelemetry;
exports.resolvePiCommand = resolvePiCommand;
exports.startPiAgent = startPiAgent;
exports.isHardPiFailure = isHardPiFailure;
exports.isTransientPiFailure = isTransientPiFailure;
exports.shouldRetryPiFailure = shouldRetryPiFailure;
...
// tools/sessions is still CJS (not converted in this wave);
// require keeps it untyped (any) without pulling non-included .js into
// the typecheck program.
const _require = (0, node_module_1.createRequire)(__filename);
const sessions = _require('../tools/sessions');
const { buildSubagentLimitPrefix } = _require('../core/subagent-limit.js');
...
function piCommandCandidates() {
    ...
    if (process.env.NVM_BIN) {
        pushCandidate(node_path_1.default.join(process.env.NVM_BIN, 'pi'));
    }
    ...
    pushCandidate(node_path_1.default.join(node_path_1.default.dirname(process.execPath), 'pi'));
    pushCandidate('pi');
    pushCandidate(node_path_1.default.join(node_os_1.default.homedir(), '.local', 'bin', 'pi'));
...
function resolveExistingCommand(candidate) {
    ...
    for (const dir of dirs) {
        ...
        const commandPath = node_path_1.default.join(dir, candidate);
        try {
            node_fs_1.default.accessSync(commandPath, node_fs_1.default.constants.X_OK);
            return candidate;
        }
...
function buildPiInvocation({ prompt, worktree, env, resume = false, sessionId = null, model = null }) {
...
function isHardPiFailure(result) {
    if (!result) {
        return false;
    }
...
function isTransientPiFailure(result) {
    if (!result) {
        return false;
    }
    const text = failureText(result);
...
function shouldRetryPiFailure(result) {
    if (!result) {
        return false;
    }
    if (result.error && (result.error.code === 'ENOENT' || result.error.code === 'EACCES')) {
...
function extractPiSessionId(stdout) {
    if (!stdout) {
        return null;
    }
...
function extractPiTelemetry(stdout) {
    if (!stdout) {
        return null;
    }
    let lastUsage = null;
...
function startPiAgent({ prompt, worktree, env, resume = false, sessionId = null, model = null, teeOptions = {}, slug = null, role = null, maxTransientRetries = 1 }) {
    ...
    async function runInvocationWithRetry(invocation) {
        ...
        if (result) {
            result.transientRetries = attempts;
        }
        return result;
    }
    function isUnrecognizedFlagError(result) {
        if (!result) {
            return false;
        }
        const text = failureText(result);
    ...
    async function runWithFallback(invocation) {
