...
exports.collectRolloutFiles = collectRolloutFiles;
exports.extractCodexTelemetry = extractCodexTelemetry;
...
function codexSessionsDir(codexHome) {
    return node_path_1.default.join(codexHome, '.codex', 'sessions');
}
/**
...
function collectRolloutFiles(sessionsDir, { sinceMs = 0 } = {}) {
    if (!sessionsDir || !node_fs_1.default.existsSync(sessionsDir)) {
        return [];
    }
    const found = [];
    ...
    while (stack.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (mtime + 1000 < sinceMs) {
                    continue;
                } // 1s grace for clock skew
