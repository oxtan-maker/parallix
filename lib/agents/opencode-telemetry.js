...
exports.extractOpencodeTelemetryFromExport = extractOpencodeTelemetryFromExport;
exports.parseOpencodeExport = extractOpencodeTelemetryFromExport;
exports.extractOpencodeTelemetry = extractOpencodeTelemetry;
exports.getOpencodeProviderModel = getOpencodeProviderModel;
/**
...
function extractSessionId(parsed) {
    ...
    if (parsed.info && typeof parsed.info === 'object') {
        if (parsed.info.id) {
            return String(parsed.info.id);
        }
    }
    // Nested in metadata/meta/session
    if (parsed.metadata && typeof parsed.metadata === 'object') {
        if (parsed.metadata.session_id) {
            return String(parsed.metadata.session_id);
        }
        if (parsed.metadata.sessionId) {
            return String(parsed.metadata.sessionId);
        }
    }
    if (parsed.meta && typeof parsed.meta === 'object') {
        if (parsed.meta.session_id) {
            return String(parsed.meta.session_id);
        }
        if (parsed.meta.sessionId) {
            return String(parsed.meta.sessionId);
        }
    }
    if (parsed.session && typeof parsed.session === 'object') {
        if (parsed.session.id) {
            return String(parsed.session.id);
        }
        if (parsed.session.session_id) {
            return String(parsed.session.session_id);
        }
    }
    return null;
}
...
function extractModelName(parsed) {
    ...
    if (parsed.model_name) {
        return String(parsed.model_name);
    }
    ...
    if (parsed.info && typeof parsed.info === 'object' && parsed.info.model && typeof parsed.info.model === 'object') {
        if (parsed.info.model.id) {
            return String(parsed.info.model.id);
        }
        if (parsed.info.model.name) {
            return String(parsed.info.model.name);
        }
    }
    // Nested in metadata/meta/session
    if (parsed.metadata && typeof parsed.metadata === 'object') {
        if (parsed.metadata.model) {
            return String(parsed.metadata.model);
        }
        if (parsed.metadata.model_name) {
            return String(parsed.metadata.model_name);
        }
    }
    if (parsed.meta && typeof parsed.meta === 'object') {
        if (parsed.meta.model) {
            return String(parsed.meta.model);
        }
    }
    if (parsed.session && typeof parsed.session === 'object') {
        if (parsed.session.model) {
            return String(parsed.session.model);
        }
    }
    return null;
}
...
function countToolCalls(parsed) {
    ...
    if (typeof parsed.tool_calls === 'number') {
        count += parsed.tool_calls;
    }
    if (typeof parsed.toolCalls === 'number') {
        count += parsed.toolCalls;
    }
    if (typeof parsed.usage?.tool_calls === 'number') {
        count += parsed.usage.tool_calls;
    }
