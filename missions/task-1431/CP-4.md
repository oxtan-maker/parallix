...
|---|---|---|
| Regression test reproduces false classification-resolution then passes after fix | `test/task-1431-integration-preflight-repro.test.js` proves preflight no longer falls back to `process.cwd()` or a stale base resolver when the selected task file already carries the correct labels; merged behavior resolves the task from the mission worktree first, falls back to the base checkout only when needed, and reads classification directly from `context.task.taskFile` | PASS |
