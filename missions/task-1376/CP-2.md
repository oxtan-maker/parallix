...
**lib/commands/stats.js:602-644** — Replaced `groupBy(rows, 'implementer')` with inline loop that computes `displayKey = (row.model && String(row.model).trim()) || (row.implementer || 'unknown')`. The returned object still uses `implementer` as the property name (for backward compatibility with callers), but its value is now the display key.
...
| Weekly report displays model values | `renderWeeklyStatsReport displays model names in Agent family column` — matches `/qwen3\.5\s+1\s+2\.00/` and `/gpt-5\s+1\s+1\.00/` |
| Range report displays model values | `renderRangeStatsReport displays model names in Agent family column` — matches `/qwen3\.5\s+1\s+2\.00/` and `/llama3\s+1\s+0\.00/` |
| Cloud-agent rows unaffected | `summarizeAgentWindow handles mixed cloud + local AI rows together` — asserts `gptEntry` exists with 1 mission |
