...
- Update `.eslintignore`: delete the `lib/commands/*.js` blanket ignore line (all commands are now `.ts`)
- Update `.gitignore`: delete the `lib/commands/*.js` blanket ignore line
...
9. **ESLint ignore updated:** `.eslintignore` no longer contains a `lib/commands/*.js` line (the blanket ignore is deleted since all commands are now `.ts`).
10. **Gitignore updated:** `.gitignore` no longer contains a `lib/commands/*.js` line.
...
- **Risk:** The `.eslintignore` blanket `lib/commands/*.js` removal means ESLint will now scan all `.js` files in `lib/commands/` (compiled output). Since all four target files are being converted, compiled `.js` will be ignored by `.gitignore` and won't exist until after `build:cjs`. ESLint should only lint `.ts` files going forward.
...
- CP 5: Update `.eslintignore` and `.gitignore` (remove `lib/commands/*.js` lines). Run full static-analysis gate. Final `npm test` sweep.

## Gates
- [ ] `npm run typecheck` — zero errors
- [ ] `npm test` — all tests pass
- [ ] `./scripts/verify-local.sh static-analysis` — all three stages pass
- [ ] `git diff -M --summary <parent-commit> -- lib/commands/{draft,active,checkpoint,handoff}.js lib/commands/{draft,active,checkpoint,handoff}.ts` — rename ≥ 50% per file
- [ ] `grep -c 'module\.exports' lib/commands/{draft,active,checkpoint,handoff}.ts` — zero matches
- [ ] `node -e "require('./lib/commands/draft')"` — loads with exports intact
- [ ] `node -e "require('./lib/commands/active')"` — loads with exports intact
- [ ] `node -e "require('./lib/commands/checkpoint')"` — loads with exports intact
- [ ] `node -e "require('./lib/commands/handoff')"` — loads with exports intact
