...
- SC 9: Each converted file remains a faithful rename rather than a rewrite — `git diff -M` reports ≥ 50% similarity (i.e. ≤ 50% of the file changed) for both `mission-utils` and `verification`. Absolute line-count deltas are intentionally not constrained: import restructuring and TypeScript interface additions legitimately change totals. (This is the same faithfulness guarantee enforced by SC 11.)
