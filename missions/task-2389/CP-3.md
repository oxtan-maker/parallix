...
Added `test/mission-activity.test.ts` (26 tests) covering the shared projection and both operator renderings, and ran the mission's declared gates.
...
| Focused rendering tests cover live authoritative work, recovery-only live coordinator evidence, unknown evidence, stale evidence, blocked work, and idle work in both the TUI and status CLI paths | `test/mission-activity.test.ts` (26 tests, TUI section `AgentStrip renders …` and CLI section `px status …`); `npx tsx --test --experimental-test-module-mocks test/mission-activity.test.ts` reports 26 pass / 0 fail | PASS |
| `./scripts/verify-local.sh static-analysis` passes; if any user-facing terminology changes in authored documentation, `./scripts/verify-local.sh docs` also passes | `./scripts/verify-local.sh static-analysis` — all 4 stages PASS; `./scripts/verify-local.sh docs` — PASS (`docs/agents.md` strip sample and the new "Mission activity in `px status`" section); `./scripts/verify-local.sh all` — exit 0, 2505 pass / 0 fail (re-verified 2026-09-11) | PASS |
