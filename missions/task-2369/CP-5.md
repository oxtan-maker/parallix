...
PR-fix aggregation retains known zero values and excludes unknown values from
its denominator: `[0, 2, unknown, unknown]` reports `1.00` from two
observations; an all-unknown group reports `unavailable` with `PR fix n` of
zero.
...
| SC12/SC15/SC16 R1, R4, R5 red at baseline and green now | `missions/task-2369/CP-1.md` baseline table; `npm test -- test/task-2369-regressions.test.ts` — 14 pass, 0 fail | PASS |
...
| PR-fix averages use observations, not completed-Mission population | `test/task-2369-regressions.test.ts`, `"R7/R8: PR-fix averages exclude unknown rounds and retain known zero"` — `[0, 2, unknown, unknown]` yields `1.00` with `PR fix n = 2`; all-unknown yields `unavailable` with `PR fix n = 0` | PASS |
