...
- **`package.json`**: removed the contradictions. `"private": true` → `false`; `name`
  `"parallix"` → scoped `"@magnus/parallix"`; `description` no longer says "unpublished" and
...
- **`README.md`**: rewrote the line-3 opener from "visualBoard AI mission lifecycle" to
  ...
  `@magnus/parallix` local tarball + global install), how the operator invokes `px`, what
...
| SC2: package.json fields mutually consistent with README install story | Done | `package.json:2` (`@magnus/parallix`), `package.json:4` (description: local tarball + px), `package.json:6` (`"private": false`), `package.json:13-15` (`publishConfig.access: restricted`), `package.json:16-18` (`bin.px`) |
