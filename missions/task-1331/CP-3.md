...
- **Test update** (`test/package-persistent-data.test.js:47-48`): the scoped package name
  `@magnus/parallix` (set in CP-2) means `npm install -g` now installs under
  `node_modules/@magnus/parallix`. Updated the resolved install root accordingly. No coverage
...
| SC2: package.json fields consistent with README install story | Pass | `package.json:2` (`@magnus/parallix`), `:4` (description: px CLI, local npm tarball), `:6` (`"private": false`), `:10` (`publishConfig`), `bin.px` retained |
