...
|---|---|---|
| `npm test` passes with exit code 0 and zero failures | `npm test` output: `pass 1687`, `fail 0`, `cancelled 0`, `skipped 22` (package.json:52) | PASS |
...
| `verify-local.sh docs` gate passes | `./scripts/verify-local.sh docs` outputs `PASS: all required documentation present` | PASS |
| `verify-local.sh static-analysis` gate passes | All 3 stages pass: ESLint clean, tsc typecheck clean, test-hygiene clean | PASS |
