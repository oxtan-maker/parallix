...
test('task-1342: weekly summary total equals user_value + ai_sdlc when some missions have invalid classification strings', () => {
  const rows = [
  ...
  // 'USER_VALUE' is lowercased by isValidClassification, so it counts as user_value.
  // 'unknown' and null do not. So: total=3, userValue=2, aiSdlc=1.
  assert.match(plain, /3\s+2\s+1/);
});
