...
test('resolveVibeCommand returns bare "vibe"', () => {
  const { resolveVibeCommand } = require('../.test-runtime/lib/agents/vibe');
...
test('extractVibeSessionId returns null for no match', () => {
  const { extractVibeSessionId } = require('../.test-runtime/lib/agents/vibe');
...
test('buildVibeInvocation includes --prompt flag', () => {
  const { buildVibeInvocation } = require('../.test-runtime/lib/agents/vibe');
...
test('startVibeAgent returns invocation and resultPromise with bare name', async () => {
  const { startVibeAgent, resolveVibeCommand } = require('../.test-runtime/lib/agents/vibe');
...
test('ensureVibeHome copies config and rewrites session logging to the worktree', () => {
  ...
  try {
    ...
    const { ensureVibeHome, vibeConfigPath, vibeSessionLogDir } = require('../.test-runtime/lib/agents/vibe');
...
test('mistral module exports expected functions', () => {
  const mistral = require('../.test-runtime/lib/agents/vibe');
...
test('processResult returns telemetry null for null input', () => {
  const { processResult } = require('../.test-runtime/lib/agents/vibe');
...
test('getVibeProviderModel returns correct fallback identity', () => {
  const { getVibeProviderModel } = require('../.test-runtime/lib/agents/vibe');
