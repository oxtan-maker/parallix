...
test('legacy active adapter preserves the launch-synchronize-stats-handoff lifecycle order', async () => {
  const { runtime, calls, transitionStore } = strictRuntime();
  const adapter = new LegacyActiveAdapter('/repo', runtime, { missionTransitionStore: transitionStore as never });
  ...
  assert.deepEqual(calls, ['preflight', 'worktree', 'task', 'checkpoint', 'config', 'prompt', 'launch-record', 'safety', 'status', 'load', 'synchronize', 'model', 'telemetry', 'stats', 'handoff']);
});

test('legacy active adapter stops after a failed launch without safety, stats, or handoff', async () => {
  const { runtime, calls, transitionStore } = strictRuntime({ async selectLaunchAndRecord() { calls.push('launch-record'); return { agent: 'codex', result: { status: 1 } }; } });
  const adapter = new LegacyActiveAdapter('/repo', runtime, { missionTransitionStore: transitionStore as never });
  assert.equal(await adapter.validateSlug('task-1'), null);
...
test('legacy active adapter fails closed when the Mission authority refuses the activation', async () => {
  ...
  const adapter = new LegacyActiveAdapter('/repo', runtime, { missionTransitionStore: missingStore as never });
  assert.equal(await adapter.validateSlug('task-1'), null);
  const launch = await adapter.launch('task-1', 'codex');
  await assert.rejects(adapter.recordLaunch('task-1', launch.agent), {
    message: 'legacy task lifecycle synchronization failed',
  });
  assert.deepEqual(calls, ['preflight', 'worktree', 'task', 'checkpoint', 'config', 'prompt', 'launch-record', 'safety', 'status', 'load']);
});

test('legacy active adapter uses the injected mission transition store identity', async () => {
  const { runtime, transitionStore } = strictRuntime();
  const adapter = new LegacyActiveAdapter('/repo', runtime, { missionTransitionStore: transitionStore as never });
  assert.equal((adapter as any)._missionTransitionStore, transitionStore);
});
