...
test('backfill seeds a Review for a mission handed off before the cutover', async () => {
  await withMissionDatabase('task-bf-1', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);
    assert.equal(await readReviewState(slug, root), null, 'precondition: no Review yet');

    const result = await backfillReviewFromLegacyState(slug, root);
...
test('backfill preserves recorded stage launches so the loop does not relaunch finished stages', async () => {
  await withMissionDatabase('task-bf-2', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);
    await backfillReviewFromLegacyState(slug, root);
...
test('backfill unblocks the write path that fails closed before it', async () => {
  await withMissionDatabase('task-bf-3', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);

    const before = await writeReviewState(slug, { ...LEGACY_STATE, phase: 'reviewing' }, root);
...
test('backfill dry run reports the migration without writing', async () => {
  await withMissionDatabase('task-bf-4', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);

    const result = await backfillReviewFromLegacyState(slug, root, { apply: false });
    assert.equal(result.outcome, 'would-backfill');
    assert.equal(result.round, 3);
    assert.equal(await readReviewState(slug, root), null, 'a dry run writes nothing');
  }, { seedReview: false });
});

test('backfill leaves an existing Review untouched', async () => {
  await withMissionDatabase('task-bf-5', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);
    const before = await readReviewState(slug, root);
    assert.equal(before.round, 1, 'precondition: the seeded review is at round 1');

    const result = await backfillReviewFromLegacyState(slug, root);
    assert.equal(result.outcome, 'already-present');

    const after = await readReviewState(slug, root);
...
test('backfill is idempotent: a second run does not duplicate rounds', async () => {
  await withMissionDatabase('task-bf-6', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);
    await backfillReviewFromLegacyState(slug, root);
    const second = await backfillReviewFromLegacyState(slug, root);

    assert.equal(second.outcome, 'already-present');
    assert.equal((await readReviewState(slug, root)).round, 3);
  }, { seedReview: false });
});

test('backfill reports a mission with no legacy state instead of inventing a Review', async () => {
  await withMissionDatabase('task-bf-7', async ({ root, slug }: any) => {
    const result = await backfillReviewFromLegacyState(slug, root);
    assert.equal(result.outcome, 'no-legacy-state');
    assert.equal(await readReviewState(slug, root), null, 'no Review is invented');
  }, { seedReview: false });
});

test('backfill fails loudly on a review-state.json it cannot reconstruct a round from', async () => {
  await withMissionDatabase('task-bf-8', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir, { round: 2, phase: 'fixing' });

    const result = await backfillReviewFromLegacyState(slug, root);
    assert.equal(result.outcome, 'failed');
    assert.match(result.diagnostic, /reviewer, implementer or startedAt/);
    assert.equal(await readReviewState(slug, root), null, 'a failed backfill writes nothing');
  }, { seedReview: false });
});

test('backfill fails loudly on unparseable legacy state', async () => {
  await withMissionDatabase('task-bf-9', async ({ root, slug, missionDir }: any) => {
...
test('px review <slug> --backfill-review dispatches the migration', async () => {
  await withMissionDatabase('task-bf-10', async ({ root, slug, missionDir }: any) => {
    writeLegacyState(missionDir);
    ...
    assert.equal(await readReviewState(slug, root), null, '--dry-run writes nothing');
