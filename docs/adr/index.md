...
- `docs/adr/0045-parallax-branch-model.md` — Two integration modes (trunk-based and feature-branch), both the `local` integration mode; the merge authority is a repository configuration key (`integration.mode`); Forgejo as PR viewer only, `review` remote wiring, and branch naming conventions
- `docs/adr/0046-npm-publish-process-and-security.md` — Public npm publication for `@magnusekdahl/parallix` from the verified main SHA through GitHub Trusted Publishing, with local version allocation and matching tag/release provenance
...
- `docs/adr/0053-operational-persistence-and-authority-boundaries.md` — One operator-local SQLite authority for checked domain concepts including Mission, Review, CheckpointData, AgentRunMeasurement, SessionMarker, LaneTransitionEvent, and AgentBlock; Mission lifecycle, rather than mutable Backlog status, is the integration authority; speculative entities such as Attempt remain excluded
