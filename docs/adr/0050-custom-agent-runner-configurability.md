...
- The `custom`-family worktree-gating regression (see Context) is fixed and regression-tested (`test/runtime-matrix.test.js`, `test/e2e-mission-lifecycle.test.js`)
- The pi draft-phase tool-call undercounting is fixed and regression-tested (`test/pi-runner.test.js`)
...
- `node test/e2e-mission-lifecycle.test.js`: 6/6
- `node test/e2e-real-agent-smoke.test.js`: `runner=opencode` PASSES end-to-end (re-run on 2026-07-10 during round-1 fixing: draft 442,512ms/132,200 input tokens/9 tool calls, active 689,382ms). `runner=pi` PASSES end-to-end on the same re-run (draft 133,793ms/8,077 input tokens/10 tool calls, active 736,196ms) — this confirms the "run 3, clean rerun" result above; the `px active` checkpoint-evidence failure recorded as "run 2" earlier in this document did not reproduce on that or the subsequent re-run (Follow-up 1 tracks characterizing the pass rate with more runs, not an unresolved failure)
- `test/pi-runner.test.js`: 20/20 (added a regression test asserting the tail-buffer size fix)
...
| `test/pi-runner.test.js` | Modified | Corrected to the real CLI contract; added telemetry/session-header coverage |
| `test/agents.test.js` | Modified | Corrected a test that had pinned the pre-fix (buggy) behavior |
| `test/e2e-real-agent-smoke.test.js` | Modified | Parametrized for both runners; made `models.custom` optional; logs real duration/token benchmark lines |
