...
The original decision adopted Option C' (Lightweight Node.js Harness) with `workflow/index.js` as the CLI entry point and modules under `workflow/lib/`. The runtime now follows the canonical `domain`, `application`, `adapters`, `interfaces`, `composition`, and `entry` source layers, with `src/entry/px.ts` bundled as `build/px.mjs` (ADR 0044). The architectural principle — scripted coordination layer, agents call CLI tools rather than following documented procedures — remains in effect. The original entrypoint and module structure are superseded; the decision to adopt a lightweight Node.js harness over bash scripts or full GSD is preserved.
